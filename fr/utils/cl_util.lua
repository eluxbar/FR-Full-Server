...
function tFR.getStreetNameAtCoord(v, w, H)
...
function tFR.randomNum(d, e)
    math.randomseed(GetGameTimer() * math.random() * 2)
    return math.random(d, e)
end
soundEventCode = 0
TriggerServerEvent("FR:soundCodeServer")
RegisterNetEvent("FR:soundCode",function(aH)
    soundEventCode = aH
end)
RegisterNetEvent("FR:playClientNuiSound",function(aI, aJ, ap) -- use for cuffing and other sounds in areas
    local ax = tFR.getPlayerCoords()
    if #(ax - aI) <= ap then
        SendNUIMessage({transactionType = aJ})
    end
end)
function tFR.getNetId(aW, aX)
    if aX == nil then
        aX = ""
    end
    local aY = 0
    ...
    if not aZ then
       -- tFR.debugLog(string.format("no such entity %s", aX))
    else
        aY = NetworkGetNetworkIdFromEntity(aW)
        if aY == aW then
           -- tFR.debugLog(string.format("no such networked entity %s", aX))
        end
    end
    return aY
end
function tFR.getObjectId(a_, aX)
    ...
    if not b0 then
       -- tFR.debugLog(string.format("no object by ID %s\n%s", a_, aX))
    else
  ...
  Citizen.CreateThread(function()
    local a = module("fr-assets", "cfg/cfg_garages")
...
function tFR.getVehicleNameFromId(aZ)
    if aT[string.lower(aZ)] then
        return aT[string.lower(aZ)].name
    end
    return ""
end
function tFR.getGarageNameFromId(aZ)
    return aT[string.lower(aZ)].garageType
end
function tFR.getVehicleIdFromModel(s)
    return aU[s]
end
local a_ = math.rad
local b0 = math.cos
local b1 = math.sin
local b2 = math.abs
function tFR.rotationToDirection(b3)
    local B = a_(b3.x)
    local D = a_(b3.z)
