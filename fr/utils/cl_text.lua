function tFR.add3DTextForCoord(a, b, c, d, e)
    local function f(g)
        tFR.DrawText3D(g.coords, g.text, 0.2)
    end
    local h = tFR.generateUUID("3dtext", 8, "alphanumeric")
    tFR.createArea("3dtext_" .. h,vector3(b, c, d),e,6.0,function()end,function()end,f,{coords = vector3(b, c, d), text = a})
end
function tFR.drawFloatingHelpText(a, i)
...
function tFR.DrawText3D(i, a, j, k, l, m)
...
function tFR.DrawText(b, c, a, j, k, p, m, q)
    SetTextScale(j, j)
    SetTextFont(k or 0)
    if p then
