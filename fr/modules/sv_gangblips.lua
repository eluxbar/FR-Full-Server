...
RegisterServerEvent("FR:EnableGangBlips")
AddEventHandler("FR:EnableGangBlips", function()
    local source = source
    ...
    if playerGang then
        ...
        for _, memberID in ipairs(gangMembers) do
            if memberID ~= user_id and FR.isPlayerOnline(memberID) then
                table.insert(filteredMembers, memberID)
            end
        end

        local blipsData = GetBlipsData(filteredMembers)
        TriggerClientEvent("FR:ReceiveGangBlipsData", source, blipsData)
    end
end)
