...
cfg.GunStores={
    ...
    ["Rebel"]={
        ["_config"]={{vector3(1904.5045166016,4927.0551757812,47.918186187744),vector3(4925.6259765625,-5243.0908203125,1.524599313736)},110,5,"Rebel Gun Store",{"rebellicense.whitelisted"},true},
    ...
    ["LargeArmsDealer"]={
        ...
        ["WEAPON_MOSIN"]={"Mosin Bolt-Action",1500000,0,"N/A","w_ar_mosin",nil,1500000},
...
MySQL.createCommand("FR/get_weapons", "SELECT weapon_info FROM fr_weapon_whitelists WHERE user_id = @user_id")
MySQL.createCommand("FR/set_weapons", "UPDATE fr_weapon_whitelists SET weapon_info = @weapon_info WHERE user_id = @user_id")
MySQL.createCommand("FR/add_user", "INSERT IGNORE INTO fr_weapon_whitelists SET user_id = @user_id")
MySQL.createCommand("FR/get_all_weapons", "SELECT * FROM fr_weapon_whitelists")
MySQL.createCommand("FR/create_weapon_code", "INSERT IGNORE INTO fr_weapon_codes SET user_id = @user_id, spawncode = @spawncode, weapon_code = @weapon_code")
MySQL.createCommand("FR/remove_weapon_code", "DELETE FROM fr_weapon_codes WHERE weapon_code = @weapon_code")
MySQL.createCommand("FR/get_weapon_codes", "SELECT * FROM fr_weapon_codes")

AddEventHandler("playerJoining", function()
    local user_id = FR.getUserId(source)
    MySQL.execute("FR/add_user", {user_id = user_id})
end)
...
RegisterNetEvent("FR:getCustomWeaponsOwned")
AddEventHandler("FR:getCustomWeaponsOwned",function()
    local source = source
    local user_id = FR.getUserId(source)
    local ownedWhitelists = {}
    MySQL.query("FR/get_weapons", {user_id = user_id}, function(weaponWhitelists)
        if weaponWhitelists[1]['weapon_info'] ~= nil then
            ...
            TriggerClientEvent('FR:gotCustomWeaponsOwned', source, ownedWhitelists)
        end
    end)
end)

RegisterNetEvent("FR:requestWhitelistedUsers")
AddEventHandler("FR:requestWhitelistedUsers",function(spawncode)
    local source = source
    local user_id = FR.getUserId(source)
    local whitelistOwners = {}
    MySQL.query("FR/get_all_weapons", {}, function(weaponWhitelists)
        for k,v in pairs(weaponWhitelists) do
            if v['weapon_info'] ~= nil then
                data = json.decode(v['weapon_info'])
                for a,b in pairs(data) do
                    if b[spawncode] then
                        whitelistOwners[v['user_id']] = (exports['fr']:executeSync("SELECT username FROM fr_users WHERE id = @user_id", {user_id = v['user_id']})[1]).username
                    end
                end
            end
        end
        TriggerClientEvent('FR:getWhitelistedUsers', source, whitelistOwners)
    end)
end)

RegisterNetEvent("FR:generateWeaponAccessCode")
AddEventHandler("FR:generateWeaponAccessCode", function(spawncode, id)
    local source = source
    local user_id = FR.getUserId(source)
    local code = math.random(100000, 999999)
    print("[FR] - Weapon Code: " .. id .. " Spawn Code: " .. spawncode .. " Code: " .. code)
    MySQL.execute("FR/create_weapon_code", {user_id = id, spawncode = spawncode, weapon_code = code})
    TriggerClientEvent('FR:generatedAccessCode', source, code)
end)

RegisterCommand("genwl", function(source, args)
    if source == 0 then
        local code = math.random(100000, 999999)
        print("[FR] - Weapon Code: " .. args[1] .. " Spawn Code: " .. args[2] .. " Code: " .. code)
        MySQL.execute("FR/create_weapon_code", {user_id = args[1], spawncode = args[2], weapon_code = code})
    end
end)
...
RegisterNetEvent("FR:requestNewGunshopData")
AddEventHandler("FR:requestNewGunshopData",function()
    local source = source
    local user_id = FR.getUserId(source)
    MySQL.query("FR/get_weapons", {user_id = user_id}, function(weaponWhitelists)
        ...
        FR.getSubscriptions(user_id, function(cb, plushours, plathours)
            if cb then
                if plathours > 0 and FR.hasPermission(user_id, "vip.gunstore") then
                    for k,v in pairs(VIPWithPlat) do
                        gunstoreData["VIP"][k] = v
                    end
                end
            end
            if FR.hasPermission(user_id, 'advancedrebel.license') then
                for k,v in pairs(RebelWithAdvanced) do
                    gunstoreData["Rebel"][k] = v
                end
            end
            TriggerClientEvent('FR:recieveFilteredGunStoreData', source, gunstoreData)
        end)
    end)
end)
...
function gunStoreLogs(weaponshop, webhook, title, text)
    if weaponshop == 'policeLargeArms' or weaponshop == 'policeSmallArms' then
        FR.sendWebhook('pd-armoury', 'FR Police Armoury Logs', text)
    elseif weaponshop == 'NHS' then
        FR.sendWebhook('nhs-armoury', 'FR NHS Armoury Logs', text)
    elseif weaponshop == 'prisonArmoury' then
        FR.sendWebhook('hmp-armoury', 'FR HMP Armoury Logs', text)
    elseif weaponshop == 'LFB' then
        FR.sendWebhook('lfb-armoury', 'FR LFB Armoury Logs', text)
    end
    FR.sendWebhook(webhook,title,text)
end

RegisterNetEvent("FR:buyWeapon")
AddEventHandler("FR:buyWeapon",function(spawncode, price, name, weaponshop, purchasetype, vipstore)
    local source = source
    local user_id = FR.getUserId(source)
    local hasPerm = false
    local gunstoreData = deepcopy(cfg.GunStores)
            ...
            if k == '_config' then
                ...
                if json.encode(v[5]) ~= '[""]' then
                    ...
                    for a,b in pairs(v[5]) do
                        if FR.hasPermission(user_id, b) then
                    ...
                    for c,d in pairs(gunstoreData[weaponshop]) do
                        if c ~= '_config' then
                            if hasPerm then
                                    ...
                                    if name == d[1] then
                                        if purchasetype == 'armour' then
                                            if string.find(spawncode, "fillUp") then
                                                ...
                                                if FR.tryPayment(user_id,price) then
                                                    FRclient.notify(source, {'~g~You bought '..name..' for £'..getMoneyStringFormatted(price)..'.'})
                                                    TriggerClientEvent("fr:PlaySound", source, "playMoney")
                                                    FRclient.setArmour(source, {100, true})
                                                    gunStoreLogs(weaponshop, 'weapon-shops',"FR Weapon Shop Logs", "> Player Name: **"..FR.getPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Purchased: **"..name.."**\n> Price: **£"..getMoneyStringFormatted(price).."**\n> Weapon Shop: **"..weaponshop.."**\n> Purchase Type: **"..purchasetype.."**")
                                                    return
                                                end
                                            elseif GetPedArmour(GetPlayerPed(source)) >= (price/1000) then
                                                FRclient.notify(source, {'You already have '..GetPedArmour(GetPlayerPed(source))..'% armour.'})
                                                return
                                            end
                                            if FR.tryPayment(user_id,d[2]) then
                                                FRclient.notify(source, {'~g~You bought '..name..' for £'..getMoneyStringFormatted(price)..'.'})
                                                TriggerClientEvent("fr:PlaySound", source, "playMoney")
                                                FRclient.setArmour(source, {price/1000, true})
                                                gunStoreLogs(weaponshop, 'weapon-shops',"FR Weapon Shop Logs", "> Player Name: **"..FR.getPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Purchased: **"..name.."**\n> Price: **£"..getMoneyStringFormatted(price).."**\n> Weapon Shop: **"..weaponshop.."**\n> Purchase Type: **"..purchasetype.."**")
                                                if weaponshop == 'LargeArmsDealer' then
                                                    FR.turfSaleToGangFunds(price, 'LargeArms')
                                                end
                                            else
                                                FRclient.notify(source, {'You do not have enough money for this purchase.'})
                                                TriggerClientEvent("fr:PlaySound", source, 2)
                                            end
                                        elseif purchasetype == 'weapon' then
                                            FRclient.hasWeapon(source, {spawncode}, function(hasWeapon)
                                                if hasWeapon then
                                                    FRclient.notify(source, {'You must store your current '..name..' before purchasing another.'})
                                                else
                                                    if FR.tryPayment(user_id,d[2]) then
                                                        if price > 0 then
                                                            FRclient.notify(source, {'~g~You bought a '..name..' for £'..getMoneyStringFormatted(price)..'.'})
                                                            if weaponshop == 'LargeArmsDealer' then
                                                                FR.turfSaleToGangFunds(price, 'LargeArms')
                                                            end
                                                        else
                                                            FRclient.notify(source, {'~g~'..name..' purchased.'})
                                                        end
                                                        TriggerClientEvent("fr:PlaySound", source, "playMoney")
                                                        FRclient.giveWeapons(source, {{[spawncode] = {ammo = 250}}, false})
                                                        gunStoreLogs(weaponshop, 'weapon-shops',"FR Weapon Shop Logs", "> Player Name: **"..FR.getPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Purchased: **"..name.."**\n> Price: **£"..getMoneyStringFormatted(price).."**\n> Weapon Shop: **"..weaponshop.."**\n> Purchase Type: **"..purchasetype.."**")
                                                    else
                                                        FRclient.notify(source, {'You do not have enough money for this purchase.'})
                                                        TriggerClientEvent("fr:PlaySound", source, 2)
                                        ...
                                        elseif purchasetype == 'ammo' then
                                            ...
                                            if FR.tryPayment(user_id,price) then
                                                if price > 0 then
                                                    FRclient.notify(source, {'~g~You bought 250x Ammo for £'..getMoneyStringFormatted(price)..'.'})
                                                    if weaponshop == 'LargeArmsDealer' then
                                                        FR.turfSaleToGangFunds(price, 'LargeArms')
                                                    end
                                                else
                                                    FRclient.notify(source, {'~g~250x Ammo purchased.'})
                                                end
                                                TriggerClientEvent("fr:PlaySound", source, "playMoney")
                                                FRclient.giveWeapons(source, {{[spawncode] = {ammo = 250}}, false})
                                                gunStoreLogs(weaponshop, 'weapon-shops',"FR Weapon Shop Logs", "> Player Name: **"..FR.getPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Purchased: **"..name.."**\n> Price: **£"..getMoneyStringFormatted(price).."**\n> Weapon Shop: **"..weaponshop.."**\n> Purchase Type: **"..purchasetype.."**")
                                ...
                                if weaponshop == 'policeLargeArms' or weaponshop == 'policeSmallArms' then
                                    FRclient.notify(source, {"You shouldn't be in here, ALARM TRIGGERED!!!"})
                                else
                                    FRclient.notify(source, {"You do not have permission to access this store."})
                                end
                            end
                        end
                    end
