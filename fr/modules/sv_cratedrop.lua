...
AddEventHandler("FR:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
       if #activeCrates > 0 then
            for k,v in pairs(activeCrates) do
                TriggerClientEvent('FR:addCrateDropRedzone', source, v, crateLocations[v])
            end
       end
    end
end)

RegisterServerEvent('FR:openCrate', function(crateID)
    local source = source
    local user_id = FR.getUserId(source)
    if activeCrates[crateID] == nil then return end
    if activeCrates[crateID].timeTillOpen > 0 then
        FRclient.notify(source, {'~r~Loot crate unlocking in '..activeCrates[crateID].timeTillOpen..' seconds.'})
    else
        if (#(GetEntityCoords(GetPlayerPed(source)) - crateLocations[crateID]) < 2.0) or (#(GetEntityCoords(GetPlayerPed(source)) - rigLocations[crateID]) < 2.0) then
            TriggerClientEvent("FR:removeLootcrate", -1, crateID)
            FreezeEntityPosition(GetPlayerPed(source), true)
            FRclient.startCircularProgressBar(source, {"", 15000, nil})
            local anims = {
                {'amb@medic@standing@kneel@base', 'base', 1},
                {'anim@gangops@facility@servers@bodysearch@', 'player_search', 1},
            }
            FRclient.playAnim(source,{true,anims,false})
            Wait(15000)
            ...
            while lootAmount > 0 do
                ...
                for k,v in pairs(availableItems) do
                    if k == randomItem then
                        FR.giveInventoryItem(user_id, v[1], v[2], true)
                    end
                end
                lootAmount = lootAmount - 1
            end
            activeCrates[crateID] = nil
            FR.giveMoney(user_id,math.random(50000,150000))
...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerClientEvent('FR:crateDrop', -1, crateCoords, crateID, false)
...
RegisterCommand('startoildrop', function(source)
    ...
    if user_id == 1 or user_id == 0 or user_id == 2 then
        ...
        TriggerClientEvent('FR:crateDrop', -1, crateCoords, crateID, true)
        ...
        FRclient.notify(source, {'You do not have permission to do this.'})
    end
end)
