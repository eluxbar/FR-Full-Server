...
RegisterNetEvent("FR:attemptBeginRoyalMailJob")
AddEventHandler("FR:attemptBeginRoyalMailJob",function()
    local source=source
    local user_id=FR.getUserId(source)
    local RoyalMailJobTable={}
    if FR.hasGroup(user_id,"Royal Mail Driver")then
        if not usersInRoyalMailJob[user_id] then
            ...
            TriggerClientEvent("FR:beginRoyalMailJob",source)
            Wait(100)
            TriggerClientEvent("FR:royalMailJobSetNextBlip",source,RoyalMailDrops[RoyalMailJobTable.stopNumber].dropPosition)
        else
            FRclient.notify(source,{"You are already a Royal Mail Driver."})
        end
    else
        FRclient.notify(source,{"~r~You are not clocked on as a Royal Mail Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(RoyalMailDrops)do
                    if FR.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(FR.getUserSource(k)))-RoyalMailDrops[v.currentJob.stopNumber].dropPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            FR.giveBankMoney(k,pay)
                            TriggerClientEvent("FR:royalMailReachedNextStop",FR.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition,pay)
                            if v.currentJob.stopNumber==#RoyalMailDrops then
                                TriggerClientEvent("FR:royalMailJobEnd", FR.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInRoyalMailJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("FR:royalMailJobSetNextBlip",FR.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition)
                            end
                        end
                    end
                end
