...
AddEventHandler("LSC:buttonSelected", function(name, button)
	...
	if user_id then
		TriggerClientEvent("LSC:buttonSelected", source, name, button, FR.tryFullPayment(FR.getUserId(source), button.price or 0)) -- money
	end
end)
...
AddEventHandler("LSC:finished", function(veh)
	...
	local user_id = FR.getUserId(source)
	if user_id then
		MySQL.execute("FRls/update_vehicle_modifications", {user_id = user_id, vehicle = veh.spawncode, modifications = json.encode({color = veh.color, extraColor = veh.extracolor, neon = veh.neon, neonColor = veh.neoncolor, xenonColor = veh.xenoncolor, smokeColor = veh.smokecolor, wheelType = veh.wheeltype, bulletProofTyres = veh.bulletProofTyres, windowTint = veh.windowtint, plateIndex = veh.plateindex, mods = veh.mods, biometric = veh.securityBiometricLock, remoteblips = veh.remoteVehicleBlips, dashcam = veh.remoteDashcams, customexhausts = veh.engineSounds})})
	end
end)
...
AddEventHandler("LSC:applyModifications", function(model, vehicle)
	...
	if model and vehicle and user_id then
		local rows = MySQL.query("FRls/get_vehicle_modifications", {user_id = user_id, vehicle = model}, function(rows, affected) 
			if rows ~= nil then 
				if #rows > 0 then
...
RegisterServerEvent("FR:setBiometricUsersState")
AddEventHandler("FR:setBiometricUsersState", function(entity, table)
	local source = source
	local user_id = FR.getUserId(source)
