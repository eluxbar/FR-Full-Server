...
RegisterNetEvent("FR:toggleShiftG4S")
AddEventHandler("FR:toggleShiftG4S", function(Status)
    local source=source
    local user_id=FR.getUserId(source)
    if FR.hasGroup(user_id,"G4S Driver")then
        if Status then
            currentUsersOnG4SJob[user_id]={currentJob=""}
            TriggerClientEvent("FR:startShiftG4S",source,math.random(1000,9999))
            jobsCompleted[user_id] = 0
            Wait(10000)
            TriggerClientEvent("FR:updateJobInformation",source,GetCollection(user_id))
        else
            if currentUsersOnG4SJob[user_id]~=nil then
                currentUsersOnG4SJob[user_id]=nil
                jobsCompleted[user_id] = nil
                TriggerClientEvent("FR:endShiftG4S",source)
            end
        end
    else
        FRclient.notify(source,{"~r~You are not clocked on as currentUsersOnG4SJob G4S Driver."})
    end
end)

RegisterNetEvent("FR:acceptJob")
AddEventHandler("FR:acceptJob",function()
    local source=source
    local user_id=FR.getUserId(source)
    if FR.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
            if k == user_id then    
                ...
                TriggerClientEvent("FR:updateJobInformation",source,v.currentJob)
            end
        end
    else
        FRclient.notify(source,{"~r~You are not clocked on as currentUsersOnG4SJob G4S Driver."})
...
RegisterNetEvent("FR:updateMoneyInVehicle")
AddEventHandler("FR:updateMoneyInVehicle", function()
    local source=source
    local user_id=FR.getUserId(source)
    if FR.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
...
RegisterNetEvent("FR:updateMoneyOutVehicle")
AddEventHandler("FR:updateMoneyOutVehicle", function()
    local source=source
    local user_id=FR.getUserId(source)
    if FR.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
    ...
    while true do
        ...
        for k,v in pairs(currentUsersOnG4SJob)do
            if v.currentJob.jobActive then
                if FR.getUserSource(k)then
                    if not v.currentJob.collected then
                        if #(GetEntityCoords(GetPlayerPed(FR.getUserSource(k)))-v.currentJob.collectionCoords)<1.5 then
                            v.currentJob.collected=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("FR:updateJobInformation",FR.getUserSource(k),v.currentJob)
                            TriggerClientEvent("FR:requestMoneyInVehicle",FR.getUserSource(k))
                        end
                    elseif v.currentJob.moneyInVehicle and not v.currentJob.depositing and not v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(FR.getUserSource(k)))-v.currentJob.depositCoords)<20 then
                            v.currentJob.depositing=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("FR:updateJobInformation",FR.getUserSource(k),v.currentJob)
                            TriggerClientEvent("FR:requestMoneyOutVehicle",FR.getUserSource(k))
                        end
                    elseif not v.currentJob.deposited and v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(FR.getUserSource(k)))-v.currentJob.depositCoords)<1.5 then
                            v.currentJob.deposited=true
                            v.currentJob.depositing=false
                            TriggerClientEvent("FR:updateJobInformation",FR.getUserSource(k),GetCollection(k))
                            local pay = grindBoost*math.random(8000,12000)
                            FR.giveBankMoney(k,pay)
                        end
                    end
                end
            end
...
RegisterCommand('g4s', function(source, args)
    ...
    for k,v in pairs(currentUsersOnG4SJob) do
        if k == user_id then
            TriggerClientEvent("FR:openG4SMenu", source)
        end
    end
end)