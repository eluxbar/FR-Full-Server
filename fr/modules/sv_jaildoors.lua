...
AddEventHandler("FR:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
        TriggerClientEvent('FR:policeSyncAllDoors', source, doors, doorStates)
    end
end)

RegisterNetEvent("FR:policeSetDoorState")
AddEventHandler("FR:policeSetDoorState", function(doorHash, state)
    local source = source
    local user_id = FR.getUserId(source)
    if FR.hasPermission(user_id, 'police.armoury') then
        doorStates[doorHash] = state
        TriggerClientEvent('FR:policeSyncDoor', -1, doorHash, state)
    end
end)

RegisterNetEvent("FR:enterPoliceSyncDoors")
AddEventHandler("FR:enterPoliceSyncDoors", function()
    local source = source
    TriggerClientEvent('FR:gotAllDoorStates', source, doorStates)
end)