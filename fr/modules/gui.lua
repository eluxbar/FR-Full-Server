...
function FR.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  FRclient.openMenuData(source,{menudata})
end

-- force close player menu
function FR.closeMenu(source)
  FRclient.closeMenu(source,{})
end
...
function FR.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  FRclient.prompt(source,{title,default_text})
end
...
function FR.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  FRclient.request(source,{id,text,time}) -- send request to client
...
function FR.registerMenuBuilder(name, builder)
...
function FR.buildMenu(name, data, cbr)
...
function FR.openMainMenu(source)
  FR.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    FR.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tFR.closeMenu(id)
...
function tFR.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tFR.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tFR.requestResult(id,ok)
...
function tFR.openMainMenu()
  FR.openMainMenu(source)
end
...
function FR.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for FR.addStaticMenuChoices calls
...
AddEventHandler("FR:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    FRclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
