! function(modules) {
    ...
    function Sequence(options, callback) {
        "function" == typeof options && (callback = options, options = {}), EventEmitter.call(this), options = options || {}, this._callback = callback, this._callSite = null, this._ended = !1, this._timeout = options.timeout, this._timer = new Timer(this)
    }
    module.exports = Sequence, Util.inherits(Sequence, EventEmitter), Sequence.determinePacket = function(byte) {
        ...
        if (!this._ended) {
            this._ended = !0, err && this._addLongStackTrace(err), this._callSite = null;
            try {
    ...
    }, Sequence.prototype.start = function() {}, Sequence.prototype._addLongStackTrace = function(err) {
        var callSiteStack = this._callSite && this._callSite.stack;
        if (callSiteStack && "string" == typeof callSiteStack && -1 === err.stack.indexOf("\n    --------------------\n")) {
            var index = callSiteStack.indexOf("\n"); - 1 !== index && (err.stack += "\n    --------------------\n" + callSiteStack.substr(index + 1))
        }
    }, Sequence.prototype._onTimeout = function() {
        this.emit("timeout")
    }
}, function(module, exports) {
    ...
    module.exports = ConnectionConfig, ConnectionConfig.mergeFlags = function(defaultFlags, userFlags) {
        ...
        if (void 0 === num) throw new TypeError("Unknown charset '" + charset + "'");
        return num
        ...
        return options && void 0 !== options.localInfile && !options.localInfile && defaultFlags.push("-LOCAL_FILES"), options && options.multipleStatements && defaultFlags.push("+MULTI_STATEMENTS"), defaultFlags
    }, ConnectionConfig.getSSLProfile = function(name) {
        SSLProfiles || (SSLProfiles = __webpack_require__(34));
        var ssl = SSLProfiles[name];
        if (void 0 === ssl) throw new TypeError("Unknown SSL profile '" + name + "'");
        return ssl
        ...
        var options = {
            host: (url = urlParse(url, !0)).hostname,
            port: url.port,
    ...
    function Connection(options) {
        Events.EventEmitter.call(this), this.config = options.config, this._socket = options.socket, this._protocol = new Protocol({
            config: this.config,
            connection: this
    ...
    function wrapCallbackInDomain(ee, fn) {
        if ("function" == typeof fn) {
            if (fn.domain) return fn;
            var domain = process.domain;
            return domain ? domain.bind(fn) : ee ? function(fn) {
                return function() {
                    for (var ret, domains = []; process.domain;) domains.shift(process.domain), process.domain.exit();
                    try {
                        ret = fn.apply(this, arguments)
                    } finally {
                        for (var i = 0; i < domains.length; i++) domains[i].enter()
                    }
                    return ret
                }
            }(wrapToDomain(ee, fn)) : fn
        }
    }

    function wrapToDomain(ee, fn) {
        return function() {
            Events.usingDomains && ee.domain ? (ee.domain.enter(), fn.apply(this, arguments), ee.domain.exit()) : fn.apply(this, arguments)
        }
    }
    module.exports = Connection, Util.inherits(Connection, Events.EventEmitter), Connection.createQuery = function(sql, values, callback) {
        ...
        var cb = callback,
            options = {};
        if ("function" == typeof sql ? cb = sql : "object" == typeof sql ? (options = Object.create(sql), "function" == typeof values ? cb = values : void 0 !== values && Object.defineProperty(options, "values", {
                value: values
            })) : (options.sql = sql, "function" == typeof values ? cb = values : void 0 !== values && (options.values = values)), void 0 !== cb && void 0 === (cb = wrapCallbackInDomain(null, cb))) throw new TypeError("argument callback must be a function when provided");
        ...
        if (callback || "function" != typeof options || (callback = options, options = {}), !this._connectCalled) {
            ...
            var connection = this;
            if (this._protocol.on("data", (function(data) {
                ...
                this._socket.setTimeout(this.config.connectTimeout, handleConnectTimeout), this._socket.once("connect", (function() {
        ...
        callback || "function" != typeof options || (callback = options, options = {}), this._implyConnect();
    ...
    }, Connection.prototype.beginTransaction = function(options, callback) {
        return callback || "function" != typeof options || (callback = options, options = {}), (options = options || {}).sql = "START TRANSACTION", options.values = null, this.query(options, callback)
    }, Connection.prototype.commit = function(options, callback) {
        return callback || "function" != typeof options || (callback = options, options = {}), (options = options || {}).sql = "COMMIT", options.values = null, this.query(options, callback)
    }, Connection.prototype.rollback = function(options, callback) {
        return callback || "function" != typeof options || (callback = options, options = {}), (options = options || {}).sql = "ROLLBACK", options.values = null, this.query(options, callback)
    }, Connection.prototype.query = function(sql, values, cb) {
        var query = Connection.createQuery(sql, values, cb);
        return query._connection = this, "object" == typeof sql && "typeCast" in sql || (query.typeCast = this.config.typeCast), query.sql && (query.sql = this.format(query.sql, query.values)), query._callback && (query._callback = wrapCallbackInDomain(this, query._callback)), this._implyConnect(), this._protocol._enqueue(query)
    }, Connection.prototype.ping = function(options, callback) {
        callback || "function" != typeof options || (callback = options, options = {}), this._implyConnect(), this._protocol.ping(options, wrapCallbackInDomain(this, callback))
    }, Connection.prototype.statistics = function(options, callback) {
        callback || "function" != typeof options || (callback = options, options = {}), this._implyConnect(), this._protocol.stats(options, wrapCallbackInDomain(this, callback))
    }, Connection.prototype.end = function(options, callback) {
        var cb = callback,
            opts = options;
        ...
        return SqlString.escape(value, !1, this.config.timezone)
    }, Connection.prototype.escapeId = function(value) {
        return SqlString.escapeId(value, !1)
    }, Connection.prototype.format = function(sql, values) {
        return "function" == typeof this.config.queryFormat ? this.config.queryFormat.call(this, sql, values, this.config.timezone) : SqlString.format(sql, values, this.config.stringifyObjects, this.config.timezone)
    }, tls.TLSSocket ? Connection.prototype._startTLS = function(onSecure) {
        var connection = this;
        ! function(config, cb) {
            var context = null,
                error = null;
            ...
            if (err) onSecure(err);
                ...
                var rejectUnauthorized = connection.config.ssl.rejectUnauthorized,
                    secureEstablished = !1,
                    secureSocket = new tls.TLSSocket(connection._socket, {
                        rejectUnauthorized,
                        requestCert: !0,
                        secureContext,
                        isServer: !1
                ...
                secureSocket.on("_tlsError", (function(err) {
                    secureEstablished ? connection._handleNetworkError(err) : onSecure(err)
                })), secureSocket.pipe(connection._protocol), connection._protocol.on("data", (function(data) {
                    secureSocket.write(data)
                })), secureSocket.on("secure", (function() {
                    secureEstablished = !0, onSecure(rejectUnauthorized ? this.ssl.verifyError() : null)
                })), secureSocket._start()
            }
        }))
        ...
        securePair.on("error", (function(err) {
            secureEstablished ? connection._handleNetworkError(err) : onSecure(err)
        })), this._socket.removeAllListeners("data"), this._protocol.removeAllListeners("data"), securePair.encrypted.pipe(this._socket), this._socket.on("data", (function(data) {
            securePair.encrypted.write(data)
        })), securePair.cleartext.pipe(this._protocol), this._protocol.on("data", (function(data) {
        ...
        })), securePair._cycle = securePair.cycle, securePair.cycle = function() {
        ...
        this.state = "protocol_error", this.emit("error", err)
    ...
    }, Connection.prototype._handleProtocolInitialize = function(packet) {
        this.threadId = packet.threadId
    }, Connection.prototype._handleProtocolEnd = function(err) {
        this.state = "disconnected", this.emit("end", err)
    }, Connection.prototype._handleProtocolEnqueue = function(sequence) {
    ...
    exports.createConnection = function(config) {
        return new(loadClass("Connection"))({
            config: new(loadClass("ConnectionConfig"))(config)
        })
    }, exports.createPool = function(config) {
        return new(loadClass("Pool"))({
            config: new(loadClass("PoolConfig"))(config)
        })
    }, exports.createPoolCluster = function(config) {
        return new(loadClass("PoolCluster"))(config)
    }, exports.createQuery = function(sql, values, callback) {
        return loadClass("Connection").createQuery(sql, values, callback)
    }, exports.escape = function(value, stringifyObjects, timeZone) {
        return loadClass("SqlString").escape(value, stringifyObjects, timeZone)
    }, exports.escapeId = function(value, forbidQualified) {
        return loadClass("SqlString").escapeId(value, forbidQualified)
    }, exports.format = function(sql, values, stringifyObjects, timeZone) {
        return loadClass("SqlString").format(sql, values, stringifyObjects, timeZone)
    }, exports.raw = function(sql) {
        return loadClass("SqlString").raw(sql)
    ...
    function Field(options) {
        options = options || {}, this.parser = options.parser, this.packet = options.packet, this.db = options.packet.db, this.table = options.packet.table, this.name = options.packet.name, this.type = Types[options.packet.type], this.length = options.packet.length
    }
    module.exports = Field, Field.prototype.string = function() {
    ...
    function sha1(msg) {
        ...
        return hash.update(msg, "binary"), hash.digest("binary")
    }

    function xor(a, b) {
        a = Buffer.from(a, "binary"), b = Buffer.from(b, "binary");
        for (var result = Buffer.allocUnsafe(a.length), i = 0; i < a.length; i++) result[i] = a[i] ^ b[i];
        return result
    }
    Auth.auth = function(name, data, options) {
        ...
        return r.seed1 = (3 * r.seed1 + r.seed2) % r.max_value, r.seed2 = (r.seed1 + r.seed2 + 33) % r.max_value, r.seed1 / r.max_value_dbl
    }, Auth.scramble323 = function(message, password) {
        if (!password) return Buffer.alloc(0);
        for (var to = Buffer.allocUnsafe(8), hashPass = this.hashPassword(password), hashMessage = this.hashPassword(message.slice(0, 8)), seed1 = this.int32Read(hashPass, 0) ^ this.int32Read(hashMessage, 0), seed2 = this.int32Read(hashPass, 4) ^ this.int32Read(hashMessage, 4), r = this.randomInit(seed1, seed2), i = 0; i < 8; i++) to[i] = Math.floor(31 * this.myRnd(r)) + 64;
        var extra = Math.floor(31 * this.myRnd(r));
        for (i = 0; i < 8; i++) to[i] ^= extra;
        return to
    }, Auth.xor32 = function(a, b) {
        return [a[0] ^ b[0], a[1] ^ b[1]]
    }, Auth.add32 = function(a, b) {
        var w1 = a[1] + b[1];
        return [65535 & a[0] + b[0] + ((4294901760 & w1) >> 16), 65535 & w1]
    }, Auth.mul32 = function(a, b) {
        var w1 = a[1] * b[1];
        return [65535 & (a[1] * b[1] >> 16 & 65535) + (a[0] * b[1] & 65535) + (a[1] * b[0] & 65535), 65535 & w1]
    }, Auth.and32 = function(a, b) {
        return [a[0] & b[0], a[1] & b[1]]
    }, Auth.shl32 = function(a, b) {
        var w1 = a[1] << b;
        return [65535 & (a[0] << b | (4294901760 & w1) >> 16), 65535 & w1]
    }, Auth.int31Write = function(buffer, number, offset) {
        buffer[offset] = number[0] >> 8 & 127, buffer[offset + 1] = 255 & number[0], buffer[offset + 2] = number[1] >> 8 & 255, buffer[offset + 3] = 255 & number[1]
    ...
    function Query(options, callback) {
        Sequence.call(this, options, callback), this.sql = options.sql, this.values = options.values, this.typeCast = void 0 === options.typeCast || options.typeCast, this.nestTables = options.nestTables || !1, this._resultSet = null, this._results = [], this._fields = [], this._index = 0, this._loadError = null
    }
    module.exports = Query, Util.inherits(Query, Sequence), Query.prototype.start = function() {
        this.emit("packet", new Packets.ComQueryPacket(this.sql))
    }, Query.prototype.determinePacket = function(byte, parser) {
        ...
        try {
            this._callback ? (this._results.push(packet), this._fields.push(void 0)) : this.emit("result", packet, this._index)
        } finally {
        ...
        this._connection.config.clientFlags & ClientConstants.CLIENT_LOCAL_FILES ? this._sendLocalDataFile(packet.filename) : (this._loadError = new Error("Load local files command is disabled"), this._loadError.code = "LOCAL_FILES_DISABLED", this._loadError.fatal = !1, this.emit("packet", new Packets.EmptyPacket))
    }, Query.prototype.ResultSetHeaderPacket = function(packet) {
        this._resultSet = new ResultSet(packet)
    }, Query.prototype.FieldPacket = function(packet) {
        this._resultSet.fieldPackets.push(packet)
    }, Query.prototype.EofPacket = function(packet) {
        this._resultSet.eofPackets.push(packet), 1 !== this._resultSet.eofPackets.length || this._callback || this.emit("fields", this._resultSet.fieldPackets, this._index), 2 === this._resultSet.eofPackets.length && (this._callback && (this._results.push(this._resultSet.rows), this._fields.push(this._resultSet.fieldPackets)), this._index++, this._resultSet = null, this._handleFinalResultPacket(packet))
        ...
        packet.parse(parser, this._resultSet.fieldPackets, this.typeCast, this.nestTables, connection), this._callback ? this._resultSet.rows.push(packet) : this.emit("result", packet, this._index)
    }, Query.prototype._sendLocalDataFile = function(path) {
        var self = this,
            localStream = fs.createReadStream(path, {
                flag: "r",
                encoding: null,
                autoClose: !0
            });
        this.on("pause", (function() {
            ...
            self._loadError = err, localStream.emit("end")
        ...
        return stream._read = function() {
            ...
            stream.push(row) || self._connection.pause(), stream.emit("result", row, i)
    ...
    Object.defineProperty(Readable.prototype, "destroyed", {
        ...
        return StringDecoder || (StringDecoder = __webpack_require__(25).StringDecoder), this._readableState.decoder = new StringDecoder(enc), this._readableState.encoding = enc, this
    };
    ...
    util.inherits(Writable, Stream), WritableState.prototype.getBuffer = function() {
        ...
        function() {
            ...
            return isBuf && !Buffer.isBuffer(chunk) && (chunk = function(chunk) {
                return Buffer.from(chunk)
            }(chunk)), "function" == typeof encoding && (cb = encoding, encoding = null), isBuf ? encoding = "buffer" : encoding || (encoding = state.defaultEncoding), "function" != typeof cb && (cb = nop), state.ended ? function(stream, cb) {
    ...
    util.inherits = __webpack_require__(8), util.inherits(Transform, Duplex), Transform.prototype.push = function(chunk, encoding) {
    ...
    function Pool(options) {
        EventEmitter.call(this), this.config = options.config, this.config.connectionConfig.pool = this, this._acquiringConnections = [], this._allConnections = [], this._freeConnections = [], this._connectionQueue = [], this._closed = !1
    }

    function spliceConnection(array, connection) {
        var index; - 1 !== (index = array.indexOf(connection)) && array.splice(index, 1)
    }
    module.exports = Pool, Util.inherits(Pool, EventEmitter), Pool.prototype.getConnection = function(cb) {
        if (this._closed) {
            var err = new Error("Pool is closed.");
            return err.code = "POOL_CLOSED", void process.nextTick((function() {
                cb(err)
            }))
        }
        var connection, pool = this;
        return this._freeConnections.length > 0 ? (connection = this._freeConnections.shift(), void this.acquireConnection(connection, cb)) : 0 === this.config.connectionLimit || this._allConnections.length < this.config.connectionLimit ? (connection = new PoolConnection(this, {
            config: this.config.newConnectionConfig()
        }), this._acquiringConnections.push(connection), this._allConnections.push(connection), void connection.connect({
        ...
        this._acquiringConnections.push(connection), changeUser ? (connection.config = this.config.newConnectionConfig(), connection.changeUser({
            timeout: this.config.acquireTimeout
        }, onOperationComplete)) : connection.ping({
            timeout: this.config.acquireTimeout
        }, onOperationComplete)
        ...
        if (-1 === this._acquiringConnections.indexOf(connection)) {
            ...
            this._closed ? this._connectionQueue.splice(0).forEach((function(cb) {
                var err = new Error("Pool is closed.");
                err.code = "POOL_CLOSED", process.nextTick((function() {
                    cb(err)
                }))
            })) : this._connectionQueue.length && this.getConnection(this._connectionQueue.shift())
        }
    }, Pool.prototype.end = function(cb) {
        this._closed = !0, "function" != typeof cb && (cb = function(err) {
            if (err) throw err
        });
        var calledBack = !1,
            waitingClose = 0;
        ...
        function onEnd(err) {
            !calledBack && (err || --waitingClose <= 0) && (calledBack = !0, cb(err))
        }
    ...
    }, Pool.prototype._purgeConnection = function(connection, callback) {
        var cb = callback || function() {};
        "disconnected" === connection.state && connection.destroy(), this._removeConnection(connection), "disconnected" === connection.state || connection._protocol._quitSequence ? process.nextTick(cb) : connection._realEnd(cb)
    }, Pool.prototype._removeConnection = function(connection) {
        connection._pool = null, spliceConnection(this._allConnections, connection), spliceConnection(this._freeConnections, connection), this.releaseConnection(connection)
    }, Pool.prototype.escape = function(value) {
        return mysql.escape(value, this.config.connectionConfig.stringifyObjects, this.config.connectionConfig.timezone)
    }, Pool.prototype.escapeId = function(value) {
        return mysql.escapeId(value, !1)
    }
}, function(module, exports, __webpack_require__) {
    ...
    module.exports = PoolConfig, PoolConfig.prototype.newConnectionConfig = function() {
        var connectionConfig = new ConnectionConfig(this.connectionConfig);
        return connectionConfig.clientFlags = this.connectionConfig.clientFlags, connectionConfig.maxPacketSize = this.connectionConfig.maxPacketSize, connectionConfig
    }
}, function(module, exports) {
    var PoolSelector = module.exports = {};
    PoolSelector.RR = function() {
        var index = 0;
        return function(clusterIds) {
            return index >= clusterIds.length && (index = 0), clusterIds[index++]
        }
    }, PoolSelector.RANDOM = function() {
        return function(clusterIds) {
            return clusterIds[Math.floor(Math.random() * clusterIds.length)]
        }
    }, PoolSelector.ORDER = function() {
        return function(clusterIds) {
            return clusterIds[0]
        }
    }
    ...
    function Protocol(options) {
        Stream.call(this), options = options || {}, this.readable = !0, this.writable = !0, this._config = options.config || {}, this._connection = options.connection, this._callback = null, this._fatalError = null, this._quitSequence = null, this._handshake = !1, this._handshaked = !1, this._ended = !1, this._destroyed = !1, this._queue = [], this._handshakeInitializationPacket = null, this._parser = new Parser({
            onError: this.handleParserError.bind(this),
            onPacket: this._parsePacket.bind(this),
            config: this._config
        })
    }
    module.exports = Protocol, Util.inherits(Protocol, Stream), Protocol.prototype.write = function(buffer) {
        return this._parser.write(buffer), !0
    }, Protocol.prototype.handshake = function(options, callback) {
        "function" == typeof options && (callback = options, options = {}), (options = options || {}).config = this._config;
        var sequence = this._enqueue(new Sequences.Handshake(options, callback));
        return this._handshake = !0, sequence
    }, Protocol.prototype.query = function(options, callback) {
        ...
        return "function" == typeof options && (callback = options, options = {}), this._enqueue(new Sequences.Ping(options, callback))
    }, Protocol.prototype.stats = function(options, callback) {
        return "function" == typeof options && (callback = options, options = {}), this._enqueue(new Sequences.Statistics(options, callback))
        ...
        return sequence.on("end", (function() {
            self.end()
        })), this._quitSequence = sequence
        ...
        var self = this;
        return sequence.on("error", (function(err) {
            self._delegateError(err, sequence)
        })).on("packet", (function(packet) {
            sequence._timer.active(), self._emitPacket(packet)
        })).on("timeout", (function() {
            ...
            self._handshaked = !0, self._fatalError || self.emit("handshake", self._handshakeInitializationPacket)
        }))), sequence.on("end", (function() {
            self._dequeue(sequence)
        })), 1 === this._queue.length && (this._parser.resetPacketNumber(), this._startSequence(sequence)), sequence
        ...
        var self = this;
        return err.fatal = !1, sequence.on("error", (function(err) {
            self._delegateError(err, sequence)
        })), process.nextTick((function() {
            sequence.end(err)
        })), !1
    ...
    }, Protocol.prototype._parsePacketDebug = function(packet) {
        try {
            packet.parse(this._parser)
        } finally {
            this._debugPacket(!0, packet)
        }
        ...
        sequence._timeout > 0 && isFinite(sequence._timeout) && sequence._timer.start(sequence._timeout), sequence.constructor === Sequences.ChangeUser ? sequence.start(this._handshakeInitializationPacket) : sequence.start()
        ...
        if (sequence) {
            if (sequence.hasErrorHandler()) return !1;
            if (!err.fatal) return !0
        }
    ...
    function Parser(options) {
        options = options || {}, this._supportBigNumbers = options.config && options.config.supportBigNumbers, this._buffer = Buffer.alloc(0), this._nextBuffers = new BufferList, this._longPacketBuffers = new BufferList, this._offset = 0, this._packetEnd = null, this._packetHeader = null, this._packetOffset = null, this._onError = options.onError || function(err) {
            throw err
        }, this._onPacket = options.onPacket || function() {}, this._nextPacketNumber = 0, this._encoding = "utf-8", this._paused = !1
    }
    module.exports = Parser, Parser.prototype.write = function(chunk) {
        ...
        if (chunk && 0 !== chunk.length) {
            ...
            if (0 !== sliceLength) {
                buffer = Buffer.allocUnsafe(sliceLength + length), offset = 0, offset += this._buffer.copy(buffer, 0, sliceStart, sliceEnd);
                for (i = 0; i < chunks.length; i++) offset += chunks[i].copy(buffer, offset)
            } else if (chunks.length > 1) {
                buffer = Buffer.allocUnsafe(length), offset = 0;
                for (i = 0; i < chunks.length; i++) offset += chunks[i].copy(buffer, offset)
            } else buffer = chunks[0];
        ...
        if (bytes > 4) {
            ...
            throw err.offset = this._offset - this._packetOffset - 1, err.code = "PARSER_UNSIGNED_TOO_LONG", err
        }
        for (; offset >= this._offset;) value = (value << 8 | buffer[offset]) >>> 0, offset--;
        return this._offset += bytes, value
    }, Parser.prototype.parseLengthCodedString = function() {
        var length = this.parseLengthCodedNumber();
        return null === length ? null : this.parseString(length)
    }, Parser.prototype.parseLengthCodedBuffer = function() {
        var length = this.parseLengthCodedNumber();
        return null === length ? null : this.parseBuffer(length)
        ...
        if (high >>> 21) {
            if (value = BigNumber(MUL_32BIT).times(high).plus(low).toString(), this._supportBigNumbers) return value;
            var err;
            throw (err = new Error('parseLengthCodedNumber: JS precision range exceeded, number is >= 53 bit: "' + value + '"')).offset = this._offset - this._packetOffset - 8, err.code = "PARSER_JS_PRECISION_RANGE_EXCEEDED", err
        }
        ...
        for (var offset = this._offset; 0 !== this._buffer[offset];)
            if (++offset >= this._buffer.length) {
                ...
                throw err.offset = this._offset - this._packetOffset, err.code = "PARSER_MISSING_NULL_BYTE", err
            } return offset
        ...
        return this._buffer.copy(response, 0, this._offset, this._offset + length), this._offset += length, response
        ...
        return this._nextPacketNumber = (this._nextPacketNumber + 1) % 256, currentPacketNumber
    ...
    }, Parser.prototype._combineNextBuffers = function(bytes) {
        ...
        var hadException = !0;
        try {
            this._onPacket(packetHeader), hadException = !1
        } catch (err) {
            if (!err || "string" != typeof err.code || "PARSER_" !== err.code.substr(0, 7)) throw err;
            this._onError(err), hadException = !1
        } finally {
            this._advanceToNextPacket(), hadException && process.nextTick(this.write.bind(this))
        }
    ...
    module.exports = function(length, number) {
        this.length = length, this.number = number
    }
}, function(__webpack_module__, __webpack_exports__, __webpack_require__) {
    "use strict";
    __webpack_require__.r(__webpack_exports__), __webpack_require__.d(__webpack_exports__, "BigNumber", (function() {
        return BigNumber
    }));
    ...
    var BigNumber = function clone(configObject) {
            ...
            if (null == b) {
                ...
                if ((isNum = "number" == typeof v) && 0 * v == 0) {
                    if (x.s = 1 / v < 0 ? (v = -v, -1) : 1, v === ~~v) {
                        ...
                        return void(e > MAX_EXP ? x.c = x.e = null : (x.e = e, x.c = [v]))
                    }
                    str = String(v)
                } else {
        ...
        function normalise(n, c, e) {
            ...
            return (e = i + 14 * e - 1) > MAX_EXP ? n.c = n.e = null : e < MIN_EXP ? n.c = [n.e = 0] : (n.e = e, n.c = c), n
        }
        ...
        function round(x, sd, rm, r) {
            ...
            if (xc) {
                out: {
                    ...
                    if (r = r || sd < 0 || null != xc[ni + 1] || (j < 0 ? n : n % pows10[d - j - 1]), r = rm < 4 ? (rd || r) && (0 == rm || rm == (x.s < 0 ? 3 : 2)) : rd > 5 || 5 == rd && (4 == rm || r || 6 == rm && (i > 0 ? j > 0 ? n / pows10[d - j] : 0 : xc[ni - 1]) % 10 & 1 || rm == (x.s < 0 ? 8 : 7)), sd < 1 || !xc[0]) return xc.length = 0,
                    r ? (sd -= x.e + 1, xc[0] = pows10[(14 - sd % 14) % 14], x.e = -sd || 0) : xc[0] = x.e = 0,
                    x;
        ...
        return BigNumber.clone = clone, BigNumber.ROUND_UP = 0, BigNumber.ROUND_DOWN = 1, BigNumber.ROUND_CEIL = 2, BigNumber.ROUND_FLOOR = 3, BigNumber.ROUND_HALF_UP = 4, BigNumber.ROUND_HALF_DOWN = 5, BigNumber.ROUND_HALF_EVEN = 6, BigNumber.ROUND_HALF_CEIL = 7, BigNumber.ROUND_HALF_FLOOR = 8, BigNumber.EUCLID = 9, BigNumber.config = BigNumber.set = function(obj) {
            ...
            if (xe = bitFloor(xe), ye = bitFloor(ye), xc = xc.slice(), a = xe - ye) {
                ...
                for (j = (xLTy = (a = xc.length) < (b = yc.length)) ? a : b, a = b = 0; b < j; b++)
                    if (xc[b] != yc[b]) {
                        xLTy = xc[b] < yc[b];
                        break
                    } if (xLTy && (t = xc, xc = yc, yc = t, y.s = -y.s), (b = (j = yc.length) - (i = xc.length)) > 0)
                for (; b--; xc[i++] = 0);
            for (b = BASE - 1; j > a;) {
                if (xc[--j] < yc[j]) {
            ...
            var q, s, x = this;
            return y = new BigNumber(y, b), !x.c || !y.s || y.c && !y.c[0] ? new BigNumber(NaN) : !y.c || x.c && !x.c[0] ? new BigNumber(x) : (9 == MODULO_MODE ? (s = y.s, y.s = 1, q = div(x, y, 0, 3), y.s = s, q.s *= s) : q = div(x, y, 0, MODULO_MODE), (y = x.minus(q.times(y))).c[0] || 1 != MODULO_MODE || (y.s = x.s), y)
            ...
            if (!(c = x.c)) return null;
            if (n = 14 * (v = c.length - 1) + 1, v = c[v]) {
                for (; v % 10 == 0; v /= 10, n--);
                for (v = c[0]; v >= 10; v /= 10, n++);
            }
            return sd && x.e + 1 > n && (n = x.e + 1), n
        }, P.shiftedBy = function(k) {
            return intCheck(k, -9007199254740991, 9007199254740991), this.times("1e" + k)
            ...
            return valueOf(this)
        }, P._isBigNumber = !0, P[Symbol.toStringTag] = "BigNumber", P[Symbol.for("nodejs.util.inspect.custom")] = P.valueOf, null != configObject && BigNumber.set(configObject), BigNumber
    }();
    ...
    module.exports = BufferList, BufferList.prototype.shift = function() {
        var buf = this.bufs.shift();
        return buf && (this.size -= buf.length), buf
    }, BufferList.prototype.push = function(buf) {
        buf && buf.length && (this.bufs.push(buf), this.size += buf.length)
    }
}, function(module, exports, __webpack_require__) {
    exports.ChangeUser = __webpack_require__(42), exports.Handshake = __webpack_require__(68), exports.Ping = __webpack_require__(69), exports.Query = __webpack_require__(20), exports.Quit = __webpack_require__(78), exports.Sequence = __webpack_require__(3), exports.Statistics = __webpack_require__(79)
}, function(module, exports, __webpack_require__) {
    ...
    function ChangeUser(options, callback) {
        Sequence.call(this, options, callback), this._user = options.user, this._password = options.password, this._database = options.database, this._charsetNumber = options.charsetNumber, this._currentConfig = options.currentConfig
    }
    module.exports = ChangeUser, Util.inherits(ChangeUser, Sequence), ChangeUser.prototype.determinePacket = function(firstByte) {
        ...
        if (void 0 !== data) this.emit("packet", new Packets.AuthSwitchResponsePacket({
            data
        }));
        else {
    ...
    function AuthSwitchRequestPacket(options) {
        options = options || {}, this.status = 254, this.authMethodName = options.authMethodName, this.authMethodData = options.authMethodData
    }
    module.exports = AuthSwitchRequestPacket, AuthSwitchRequestPacket.prototype.parse = function(parser) {
        this.status = parser.parseUnsignedNumber(1), this.authMethodName = parser.parseNullTerminatedString(), this.authMethodData = parser.parsePacketTerminatedBuffer()
    }, AuthSwitchRequestPacket.prototype.write = function(writer) {
        writer.writeUnsignedNumber(1, this.status), writer.writeNullTerminatedString(this.authMethodName), writer.writeBuffer(this.authMethodData)
    }
}, function(module, exports) {
    function AuthSwitchResponsePacket(options) {
        options = options || {}, this.data = options.data
    }
    module.exports = AuthSwitchResponsePacket, AuthSwitchResponsePacket.prototype.parse = function(parser) {
        this.data = parser.parsePacketTerminatedBuffer()
    }, AuthSwitchResponsePacket.prototype.write = function(writer) {
    ...
    function ClientAuthenticationPacket(options) {
        options = options || {}, this.clientFlags = options.clientFlags, this.maxPacketSize = options.maxPacketSize, this.charsetNumber = options.charsetNumber, this.filler = void 0, this.user = options.user, this.scrambleBuff = options.scrambleBuff, this.database = options.database, this.protocol41 = options.protocol41
    }
    module.exports = ClientAuthenticationPacket, ClientAuthenticationPacket.prototype.parse = function(parser) {
        this.protocol41 ? (this.clientFlags = parser.parseUnsignedNumber(4), this.maxPacketSize = parser.parseUnsignedNumber(4), this.charsetNumber = parser.parseUnsignedNumber(1), this.filler = parser.parseFiller(23), this.user = parser.parseNullTerminatedString(), this.scrambleBuff = parser.parseLengthCodedBuffer(), this.database = parser.parseNullTerminatedString()) : (this.clientFlags = parser.parseUnsignedNumber(2), this.maxPacketSize = parser.parseUnsignedNumber(3), this.user = parser.parseNullTerminatedString(), this.scrambleBuff = parser.parseBuffer(8), this.database = parser.parseLengthCodedBuffer())
    }, ClientAuthenticationPacket.prototype.write = function(writer) {
        this.protocol41 ? (writer.writeUnsignedNumber(4, this.clientFlags), writer.writeUnsignedNumber(4, this.maxPacketSize), writer.writeUnsignedNumber(1, this.charsetNumber), writer.writeFiller(23), writer.writeNullTerminatedString(this.user), writer.writeLengthCodedBuffer(this.scrambleBuff), writer.writeNullTerminatedString(this.database)) : (writer.writeUnsignedNumber(2, this.clientFlags), writer.writeUnsignedNumber(3, this.maxPacketSize), writer.writeNullTerminatedString(this.user), writer.writeBuffer(this.scrambleBuff), this.database && this.database.length && (writer.writeFiller(1), writer.writeBuffer(Buffer.from(this.database))))
    }
}, function(module, exports) {
    function ComChangeUserPacket(options) {
        options = options || {}, this.command = 17, this.user = options.user, this.scrambleBuff = options.scrambleBuff, this.database = options.database, this.charsetNumber = options.charsetNumber
    }
    module.exports = ComChangeUserPacket, ComChangeUserPacket.prototype.parse = function(parser) {
        this.command = parser.parseUnsignedNumber(1), this.user = parser.parseNullTerminatedString(), this.scrambleBuff = parser.parseLengthCodedBuffer(), this.database = parser.parseNullTerminatedString(), this.charsetNumber = parser.parseUnsignedNumber(1)
    }, ComChangeUserPacket.prototype.write = function(writer) {
        writer.writeUnsignedNumber(1, this.command), writer.writeNullTerminatedString(this.user), writer.writeLengthCodedBuffer(this.scrambleBuff), writer.writeNullTerminatedString(this.database), writer.writeUnsignedNumber(2, this.charsetNumber)
    }
}, function(module, exports) {
    function ComPingPacket() {
        this.command = 14
    }
    module.exports = ComPingPacket, ComPingPacket.prototype.write = function(writer) {
    ...
    function ComQueryPacket(sql) {
        this.command = 3, this.sql = sql
    }
    module.exports = ComQueryPacket, ComQueryPacket.prototype.write = function(writer) {
        writer.writeUnsignedNumber(1, this.command), writer.writeString(this.sql)
    }, ComQueryPacket.prototype.parse = function(parser) {
        this.command = parser.parseUnsignedNumber(1), this.sql = parser.parsePacketTerminatedString()
    }
}, function(module, exports) {
    function ComQuitPacket() {
        this.command = 1
    }
    module.exports = ComQuitPacket, ComQuitPacket.prototype.parse = function(parser) {
        this.command = parser.parseUnsignedNumber(1)
    ...
    module.exports = ComStatisticsPacket, ComStatisticsPacket.prototype.write = function(writer) {
        writer.writeUnsignedNumber(1, this.command)
    ...
    function EofPacket(options) {
        options = options || {}, this.fieldCount = void 0, this.warningCount = options.warningCount, this.serverStatus = options.serverStatus, this.protocol41 = options.protocol41
    }
    module.exports = EofPacket, EofPacket.prototype.parse = function(parser) {
        this.fieldCount = parser.parseUnsignedNumber(1), this.protocol41 && (this.warningCount = parser.parseUnsignedNumber(2), this.serverStatus = parser.parseUnsignedNumber(2))
    }, EofPacket.prototype.write = function(writer) {
        writer.writeUnsignedNumber(1, 254), this.protocol41 && (writer.writeUnsignedNumber(2, this.warningCount), writer.writeUnsignedNumber(2, this.serverStatus))
    }
}, function(module, exports) {
    function ErrorPacket(options) {
        options = options || {}, this.fieldCount = options.fieldCount, this.errno = options.errno, this.sqlStateMarker = options.sqlStateMarker, this.sqlState = options.sqlState, this.message = options.message
    }
    module.exports = ErrorPacket, ErrorPacket.prototype.parse = function(parser) {
        this.fieldCount = parser.parseUnsignedNumber(1), this.errno = parser.parseUnsignedNumber(2), 35 === parser.peak() && (this.sqlStateMarker = parser.parseString(1), this.sqlState = parser.parseString(5)), this.message = parser.parsePacketTerminatedString()
    }, ErrorPacket.prototype.write = function(writer) {
        writer.writeUnsignedNumber(1, 255), writer.writeUnsignedNumber(2, this.errno), this.sqlStateMarker && (writer.writeString(this.sqlStateMarker), writer.writeString(this.sqlState)), writer.writeString(this.message)
    }
}, function(module, exports) {
    function FieldPacket(options) {
        options = options || {}, this.catalog = options.catalog, this.db = options.db, this.table = options.table, this.orgTable = options.orgTable, this.name = options.name, this.orgName = options.orgName, this.charsetNr = options.charsetNr, this.length = options.length, this.type = options.type, this.flags = options.flags, this.decimals = options.decimals, this.default = options.default, this.zeroFill = options.zeroFill, this.protocol41 = options.protocol41
    }
    ...
    module.exports = OkPacket, OkPacket.prototype.parse = function(parser) {
        ...
        writer.writeUnsignedNumber(1, 0), writer.writeLengthCodedNumber(this.affectedRows || 0), writer.writeLengthCodedNumber(this.insertId || 0), this.protocol41 && (writer.writeUnsignedNumber(2, this.serverStatus || 0), writer.writeUnsignedNumber(2, this.warningCount || 0)), writer.writeString(this.message)
    }
}, function(module, exports) {
    function OldPasswordPacket(options) {
        options = options || {}, this.scrambleBuff = options.scrambleBuff
    }
    module.exports = OldPasswordPacket, OldPasswordPacket.prototype.parse = function(parser) {
    ...
    function ResultSetHeaderPacket(options) {
        options = options || {}, this.fieldCount = options.fieldCount
    }
    module.exports = ResultSetHeaderPacket, ResultSetHeaderPacket.prototype.parse = function(parser) {
    ...
    function SSLRequestPacket(options) {
        options = options || {}, this.clientFlags = options.clientFlags | ClientConstants.CLIENT_SSL, this.maxPacketSize = options.maxPacketSize, this.charsetNumber = options.charsetNumber
    }
    module.exports = SSLRequestPacket, SSLRequestPacket.prototype.parse = function(parser) {
        this.clientFlags = parser.parseUnsignedNumber(4), this.maxPacketSize = parser.parseUnsignedNumber(4), this.charsetNumber = parser.parseUnsignedNumber(1)
    }, SSLRequestPacket.prototype.write = function(writer) {
        writer.writeUnsignedNumber(4, this.clientFlags), writer.writeUnsignedNumber(4, this.maxPacketSize), writer.writeUnsignedNumber(1, this.charsetNumber), writer.writeFiller(23)
    }
}, function(module, exports) {
    ...
    function UseOldPasswordPacket(options) {
        options = options || {}, this.firstByte = options.firstByte || 254
    }
    module.exports = UseOldPasswordPacket, UseOldPasswordPacket.prototype.parse = function(parser) {
    ...
    function Timer(object) {
        this._object = object, this._timeout = null
    }
    module.exports = Timer, Timer.prototype.active = function() {
        this._timeout && (this._timeout.refresh ? this._timeout.refresh() : Timers.active(this._timeout))
    }, Timer.prototype.start = function(msecs) {
        this.stop(), this._timeout = Timers.setTimeout(this._onTimeout.bind(this), msecs)
    }, Timer.prototype.stop = function() {
        this._timeout && (Timers.clearTimeout(this._timeout), this._timeout = null)
    ...
    function Handshake(options, callback) {
        Sequence.call(this, options, callback), options = options || {}, this._config = options.config, this._handshakeInitializationPacket = null
    }
    module.exports = Handshake, Util.inherits(Handshake, Sequence), Handshake.prototype.determinePacket = function(firstByte, parser) {
        return 255 === firstByte ? Packets.ErrorPacket : this._handshakeInitializationPacket ? 254 === firstByte ? 1 === parser.packetLength() ? Packets.UseOldPasswordPacket : Packets.AuthSwitchRequestPacket : void 0 : Packets.HandshakeInitializationPacket
        ...
        if (this._config.ssl) {
            if (!serverSSLSupport) {
                ...
                return err.code = "HANDSHAKE_NO_SSL_SUPPORT", err.fatal = !0, void this.end(err)
            }
            this._config.clientFlags |= ClientConstants.CLIENT_SSL, this.emit("packet", new Packets.SSLRequestPacket({
        ...
        if (!this._config.insecureAuth) {
            ...
            return err.code = "HANDSHAKE_INSECURE_AUTH", err.fatal = !0, void this.end(err)
        }
    ...
    function Ping(options, callback) {
        callback || "function" != typeof options || (callback = options, options = {}), Sequence.call(this, options, callback)
    }
    module.exports = Ping, Util.inherits(Ping, Sequence), Ping.prototype.start = function() {
        this.emit("packet", new Packets.ComPingPacket)
    }
}, function(module, exports) {
    module.exports = function(resultSetHeaderPacket) {
        this.resultSetHeaderPacket = resultSetHeaderPacket, this.fieldPackets = [], this.eofPackets = [], this.rows = []
    }
}, function(module, exports) {
    ...
    util.inherits = __webpack_require__(8), util.inherits(PassThrough, Transform), PassThrough.prototype._transform = function(chunk, encoding, cb) {
        cb(null, chunk)
    }
}, function(module, exports, __webpack_require__) {
    var Sequence = __webpack_require__(3),
    ...
    function Quit(options, callback) {
        callback || "function" != typeof options || (callback = options, options = {}), Sequence.call(this, options, callback), this._started = !1
    }
    module.exports = Quit, Util.inherits(Quit, Sequence), Quit.prototype.end = function(err) {
        this._ended || (this._started && err && "ECONNRESET" === err.code && "read" === err.syscall ? Sequence.prototype.end.call(this) : Sequence.prototype.end.call(this, err))
    }, Quit.prototype.start = function() {
        this._started = !0, this.emit("packet", new Packets.ComQuitPacket)
    }
}, function(module, exports, __webpack_require__) {
    var Sequence = __webpack_require__(3),
        Util = __webpack_require__(0),
    ...
    module.exports = Statistics, Util.inherits(Statistics, Sequence), Statistics.prototype.start = function() {
        this.emit("packet", new Packets.ComStatisticsPacket)
    }, Statistics.prototype.StatisticsPacket = function(packet) {
        this.end(null, packet)
    }, Statistics.prototype.determinePacket = function(firstByte) {
        if (85 === firstByte) return Packets.StatisticsPacket
    }
}, function(module, exports, __webpack_require__) {
    ...
    module.exports = PacketWriter, PacketWriter.prototype.toBuffer = function(parser) {
        ...
        for (var i = 0; i < bytes; i++) this._buffer[this._offset++] = value >> 8 * i & 255
    }, PacketWriter.prototype.writeFiller = function(bytes) {
        this._allocate(bytes);
        for (var i = 0; i < bytes; i++) this._buffer[this._offset++] = 0
        ...
        this.writeBuffer(value), this.writeFiller(1)
    ...
    function escapeString(val) {
        for (var match, chunkIndex = CHARS_GLOBAL_REGEXP.lastIndex = 0, escapedVal = ""; match = CHARS_GLOBAL_REGEXP.exec(val);) escapedVal += val.slice(chunkIndex, match.index) + CHARS_ESCAPE_MAP[match[0]], chunkIndex = CHARS_GLOBAL_REGEXP.lastIndex;
        return 0 === chunkIndex ? "'" + val + "'" : chunkIndex < val.length ? "'" + escapedVal + val.slice(chunkIndex) + "'" : "'" + escapedVal + "'"
    }
    ...
    SqlString.escapeId = function(val, forbidQualified) {
        if (Array.isArray(val)) {
            for (var sql = "", i = 0; i < val.length; i++) sql += (0 === i ? "" : ", ") + SqlString.escapeId(val[i], forbidQualified);
            return sql
        }
        return forbidQualified ? "`" + String(val).replace(ID_GLOBAL_REGEXP, "``") + "`" : "`" + String(val).replace(ID_GLOBAL_REGEXP, "``").replace(QUAL_GLOBAL_REGEXP, "`.`") + "`"
        ...
        if ("local" === timeZone) year = dt.getFullYear(), month = dt.getMonth() + 1, day = dt.getDate(), hour = dt.getHours(), minute = dt.getMinutes(), second = dt.getSeconds(), millisecond = dt.getMilliseconds();
            ...
            !1 !== tz && 0 !== tz && dt.setTime(dt.getTime() + 6e4 * tz), year = dt.getUTCFullYear(), month = dt.getUTCMonth() + 1, day = dt.getUTCDate(), hour = dt.getUTCHours(), minute = dt.getUTCMinutes(), second = dt.getUTCSeconds(), millisecond = dt.getUTCMilliseconds()
        }
        ...
        for (var key in object) {
            var val = object[key];
            "function" != typeof val && (sql += (0 === sql.length ? "" : ", ") + SqlString.escapeId(key) + " = " + SqlString.escape(val, !0, timeZone))
        }
        return sql
    }, SqlString.raw = function(sql) {
        if ("string" != typeof sql) throw new TypeError("argument sql must be a string");
        return {
    ...
    function PoolConnection(pool, options) {
        Connection.call(this, options), this._pool = pool, Events.usingDomains && (this.domain = pool.domain), this.on("end", this._removeFromPool), this.on("error", (function(err) {
            err.fatal && this._removeFromPool()
        }))
    }
    module.exports = PoolConnection, inherits(PoolConnection, Connection), PoolConnection.prototype.release = function() {
        var pool = this._pool;
        if (pool && !pool._closed) return pool.releaseConnection(this)
    }, PoolConnection.prototype._realEnd = Connection.prototype.end, PoolConnection.prototype.end = function() {
        ...
        Connection.prototype.destroy.apply(this, arguments), this._removeFromPool(this)
    ...
    module.exports = PoolCluster, Util.inherits(PoolCluster, EventEmitter), PoolCluster.prototype.add = function(id, config) {
        ...
        pattern = pattern || "*", selector = (selector = selector || this._defaultSelector).toUpperCase(), void 0 === PoolSelector[selector] && (selector = this._defaultSelector);
        var key = pattern + selector;
        return void 0 === this._namespaces[key] && (this._namespaces[key] = new PoolNamespace(this, pattern, selector)), this._namespaces[key]
        ...
        var namespace;
        "function" == typeof pattern ? (cb = pattern, namespace = this.of()) : ("function" == typeof selector && (cb = selector, selector = this._defaultSelector), namespace = this.of(pattern, selector)), namespace.getConnection(cb)
        ...
        if (void 0 === foundNodeIds) {
            var expression = function(pattern) {
                if (val = pattern, "object" == typeof val && "[object RegExp]" === Object.prototype.toString.call(val)) return pattern;
                var val;
                var source = pattern.replace(/([.+?^=!:${}()|\[\]\/\\])/g, "\\$1").replace(/\*/g, ".*");
                return new RegExp("^" + source + "$")
            }(pattern);
            foundNodeIds = Object.keys(this._nodes).filter((function(id) {
                return id.match(expression)
            })), this._findCaches[pattern] = foundNodeIds
        }
        return includeOffline ? foundNodeIds : foundNodeIds.filter((function(nodeId) {
            var node = this._getNode(nodeId);
            return !node._offlineUntil || (currentTime || (currentTime = getMonotonicMilliseconds()), node._offlineUntil <= currentTime)
        }), this)
        ...
        delete this._nodes[node.id], this._clearFindCaches(), node.pool.end(_noop)
    }
}, function(module, exports, __webpack_require__) {
    ...
    function PoolNamespace(cluster, pattern, selector) {
        this._cluster = cluster, this._pattern = pattern, this._selector = new PoolSelector[selector]
    }
    module.exports = PoolNamespace, PoolNamespace.prototype.getConnection = function(cb) {
        ...
        if (null === clusterNode) {
            var err = null;
            return 0 !== this._cluster._findNodeIds(this._pattern, !0).length ? (err = new Error("Pool does not have online node.")).code = "POOL_NONEONLINE" : (err = new Error("Pool does not exist.")).code = "POOL_NOEXIST", void cb(err)
        }
        cluster._getConnection(clusterNode, (function(err, connection) {
            err && cluster._canRetry && 0 !== cluster._findNodeIds(namespace._pattern).length ? namespace.getConnection(cb) : err ? cb(err) : cb(null, connection)
        }))
        ...
        if (null === clusterNode) {
            var err = null;
            return 0 !== this._cluster._findNodeIds(this._pattern, !0).length ? (err = new Error("Pool does not have online node.")).code = "POOL_NONEONLINE" : (err = new Error("Pool does not exist.")).code = "POOL_NOEXIST", process.nextTick((function() {
                query.on("error", (function() {})), query.end(err)
            })), query
        }
    ...
    var utility_safeInvoke = function(callback, args) {
        "function" == typeof callback && setImmediate(() => {
            callback(args)
        })
    };
    var utility_convertLegacyQuery = function(query, parameters) {
        let sql = query;
        const params = [];
        return sql = sql.replace(/@(\w+)/g, (txt, match) => {
            let returnValue = txt;
            return Object.prototype.hasOwnProperty.call(parameters, txt) ? (params.push(parameters[txt]), returnValue = "?") : Object.prototype.hasOwnProperty.call(parameters, match) && (params.push(parameters[match]), returnValue = "?"), returnValue
        }), {
            sql,
            params
        }
    ...
    var utility_prepareLegacyQuery = function(query, parameters) {
        let sql = query,
            params = parameters;
        return null === params || "object" != typeof params || Array.isArray(params) || ({
            sql,
            params
        } = utility_convertLegacyQuery(sql, params)), [sql, params]
    };
    ...
    var queryStore = new class {
        ...
        add(query) {
            ...
            return -1 === index ? this.queryStringStorage.push(query) - 1 : index
        }
        get(query) {
            return "number" == typeof query ? this.find(query) : query
        }
        find(storageId) {
            return this.queryStringStorage.length > storageId ? this.queryStringStorage[storageId] : "Error: Query string not found in storage"
        }
    };
    var Color, OutputDestination, utility_sanitizeTransactionInput = function(querys, params, callback) {
        ...
        return sqls = querys.map(query => "string" == typeof query || "number" == typeof query ? {
            query: queryStore.get(query),
            values
        } : {
            query: queryStore.get(query.query),
            values: query.values
        }), "function" == typeof params && (cb = params), sqls = utility_convertTransactionLegacyQueries(sqls), [sqls, cb]
    };
    ! function(Color) {
        Color.Default = "[0m", Color.Error = "[31m", Color.Success = "[32m", Color.Warning = "[33m", Color.Info = "[36m"
    }(Color || (Color = {})),
    function(OutputDestination) {
        OutputDestination[OutputDestination.None = "None"] = "None", OutputDestination[OutputDestination.File = "File"] = "File", OutputDestination[OutputDestination.Console = "Console"] = "Console", OutputDestination[OutputDestination.FileAndConsole = "FileAndConsole"] = "FileAndConsole"
    ...
    var server_mysql = class {
            constructor(mysqlConfig, profiler, logger) {
                this.pool = null, this.profiler = profiler, this.logger = logger, this.formatQuery = sql => `${sql.sql} : ${JSON.stringify(sql.values)}`, "object" == typeof mysqlConfig ? this.pool = Object(mysql.createPool)(mysqlConfig) : this.logger.error(`Unexpected configuration of type ${typeof mysqlConfig} received.`), this.pool.query("SELECT VERSION()", (error, result) => {
                    if (error) this.logger.error(error.message);
                        ...
                        const formattedVersion = function(versionString) {
                            let versionPrefix = "MariaDB";
                            const version = versionString;
                            return "5" !== version[0] && "8" !== version[0] || (versionPrefix = "MySQL"), {
                                versionPrefix,
                                version
                            }
                        }(result[0]["VERSION()"]);
                        profiler.setVersion(formattedVersion), this.logger.success("Database server connection established."), emit("onDatabaseConnect")
                    }
                })
            }
            execute(sql, invokingResource, connection) {
                return new Promise((resolve, reject) => {
                    ...
                    (connection || this.pool).query(sql, (error, result) => {
                        this.profiler.profile(process.hrtime(start), this.formatQuery(sql), invokingResource), error && reject(error), setImmediate(async () => {
                            resolve(result)
                        })
                    })
                }).catch(error => {
                    if (connection) throw this.logger.info(`[${invokingResource}] A (possible deliberate) error happens on transaction for query "${this.formatQuery(sql)}": ${error.message}`, {
                        tag: this.profiler.version
                    }), new Error("See Info-Message for full information: " + error.message);
            ...
            onTransactionError(error, connection, callback) {
                connection.rollback(() => {
                    this.logger.error(error.message), callback(!1)
                })
            }
            beginTransaction(callback) {
                this.pool.getConnection((connectionError, connection) => {
                    ...
                    connection.beginTransaction(transactionError => {
                        transactionError ? this.onTransactionError(transactionError, connection, callback) : callback(connection)
                    })
                })
            }
            commitTransaction(promises, connection, callback) {
                Promise.all(promises).then(() => {
                    connection.commit(commitError => {
                        commitError ? this.onTransactionError(commitError, connection, callback) : callback(!0)
                    })
                }).catch(executeError => {
    ...
    var logger_getTimeStamp = function() {
    ...
    ! function(LogLevel) {
        LogLevel[LogLevel.Info = 0] = "Info", LogLevel[LogLevel.Success = 1] = "Success", LogLevel[LogLevel.Warning = 2] = "Warning", LogLevel[LogLevel.Error = 3] = "Error"
    }(LogLevel || (LogLevel = {}));
    var logger_logLevel = LogLevel;
    var logger_getLoggingFileName = function() {
        return GetConvar("mysql_log_file_format", "%s-%d.log").replace("%s", GetCurrentResourceName()).replace("%d", Date.now().toString())
    };
    var server_logger = class {
        constructor(outputString, defaultOverRides) {
            ...
            this.defaultConfig = {
                ...
                output,
                ...defaultOverRides
            }, this.loggingFile = null, this.defaultConfig.output !== OutputDestination.File && this.defaultConfig.output !== OutputDestination.FileAndConsole || (this.loggingFile = logger_getLoggingFileName(), Object(external_fs_.closeSync)(Object(external_fs_.openSync)(this.loggingFile, "w")))
        }
        ...
        getOutputDestination(logLevel) {
            return 0 != (this.defaultConfig.logLevel & 1 << logLevel) ? OutputDestination.FileAndConsole : OutputDestination.File
        }
        error(msg, options = {}) {
            this.log(msg, {
                color: Color.Error,
    ...
    var server_profiler = class {
        constructor(config, logger) {
            this.version = "MySQL", this.logger = logger, this.startTime = Date.now(), this.config = {
        ...
        addSlowQuery(sql, resource, queryTime) {
            if (this.profiles.slowQueries.push({
                    sql,
                    resource,
                    queryTime
                }), this.profiles.slowQueries.length > this.config.slowestQueries) {
        ...
        setVersion({
            ...
            version.startsWith("8.0.") && "MySQL" === versionPrefix && this.logger.warning("It is recommended to run MySQL 5 or MariaDB with mysql-async. You may experience performance issues under load by using MySQL 8."), this.version = `${versionPrefix}:${version}`
        }
        fillExecutionTimes(interval) {
            for (let i = 0; i < interval; i += 1) this.profiles.executionTimes[i] || (this.profiles.executionTimes[i] = {
    ...
    var server = class {
            constructor(config, loggerOverrides = {}) {
                this.logger = new server_logger(GetConvar("mysql_debug", "None"), {
                    ...loggerOverrides,
                ...
                }, this.logger), this.mysql = new server_mysql(config, this.profiler, this.logger), this.queryStorage = queryStore
            }
            execute(query, parameters, callback, resource) {
                let sql = this.queryStorage.get(query),
                    values = parameters,
                    cb = callback;
                return [sql, values, cb] = utility_sanitizeInput(sql, values, cb), new Promise(resolve => {
                    this.mysql.execute({
                        sql,
                        values,
    ...
    }, !0), onNet("fr:request-data", () => {
        const src = source;
        emitNet("fr:update-resource-data", src, server_server.profiler.profiles.resources), emitNet("fr:update-time-data", src, server_server.profiler.profiles.executionTimes), emitNet("fr:update-slow-queries", src, server_server.profiler.profiles.slowQueries)
    }), onNet("fr:request-server-status", () => {
        const src = source;
        server_server.execute("SHOW GLOBAL STATUS", data => {
            emitNet("fr:update-status", src, data)
        }, null, "fr").then(([result, cb]) => {
            cb(result)
        }).catch(() => !1), server_server.execute("SHOW GLOBAL VARIABLES", data => {
            emitNet("fr:update-variables", src, data)
        }, null, "fr").then(([result, cb]) => {
            cb(result)
        }).catch(() => !1)
    })
