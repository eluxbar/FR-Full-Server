...
RegisterNetEvent("FR:attemptBeginDeliverooJob")
AddEventHandler("FR:attemptBeginDeliverooJob",function()
    local source=source
    local user_id=FR.getUserId(source)
    local deliverooJobTable={}
    if FR.hasGroup(user_id,"Deliveroo")then
        if not usersInDeliverooJob[user_id] then
            ...
            TriggerClientEvent("FR:beginDeliverooJob",source)
            Wait(100)
            TriggerClientEvent("FR:deliverooJobSetNextBlip",source,deliverooStops[deliverooJobTable.stopNumber].stopPosition)
        else
            FRclient.notify(source,{"You are already a Deliveroo Driver."})
        end
    else
        FRclient.notify(source,{"~r~You are not clocked on as a Deliveroo Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(deliverooStops)do
                    if FR.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(FR.getUserSource(k)))-deliverooStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            FR.giveBankMoney(k,pay)
                            TriggerClientEvent("FR:deliverooJobReachedNextStop",FR.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#deliverooStops then
                                TriggerClientEvent("FR:deliverooJobEnd", FR.getUserSource(k), true)
                                v.currentJob.jobActive=false
                                usersInDeliverooJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("FR:deliverooJobSetNextBlip",FR.getUserSource(k),deliverooStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
