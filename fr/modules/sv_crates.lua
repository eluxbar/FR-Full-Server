local lootableitems = {
    ...
    -- Plat
    {name = "FR Platinum (30 Days)", item = "fr_platinum", chance = 10},
}
...
RegisterNetEvent("FR:Crate:OpenMenu")
AddEventHandler("FR:Crate:OpenMenu", function()
    local source = source
    local user_id = FR.getUserId(source)
    exports['fr']:execute('SELECT * FROM fr_crates WHERE user_id = @user_id', { ['@user_id'] = user_id }, function(result)
        local keys = {}
        if #result > 0 then
            keys.silver = result[1].silver
            keys.gold = result[1].gold
            keys.plat = result[1].plat
            TriggerClientEvent("FR:Crate:SendData", source, keys, lootableitems)
        else
            exports['fr']:execute('INSERT INTO fr_crates (user_id, silver, gold, plat) VALUES (@user_id, @silver, @gold, @plat)', { ['@user_id'] = user_id, ['@silver'] = 0, ['@gold'] = 0, ['@plat'] = 0 })
            keys.silver = 0
            keys.gold = 0
            keys.plat = 0
            TriggerClientEvent("FR:Crate:SendData", source, keys, lootableitems)
...
RegisterNetEvent("FR:Crate:Redeem")
AddEventHandler("FR:Crate:Redeem", function(key)
    key = string.lower(key)
    local source = source
    local user_id = FR.getUserId(source)
    local item, name = GetRandomItem(key)
    exports['fr']:execute('SELECT ' .. key .. ' FROM fr_crates WHERE user_id = @user_id', { ['@user_id'] = user_id }, function(result)
        if result and result[1] then    
            local keys = result[1][key]
            if keys > 0 then
                keys = keys - 1
                exports['fr']:execute('UPDATE fr_crates SET ' .. key .. ' = @keys WHERE user_id = @user_id', { ['@keys'] = keys, ['@user_id'] = user_id })
                TriggerClientEvent("FR:Crate:StartRedeem", source, name)
                local code1, code2 = generateUUID("Items", 4, "alphanumeric"), generateUUID("Items", 4, "alphanumeric")
                local code = string.upper(code1 .. "-" .. code2)
                exports['fr']:execute("INSERT INTO fr_stores (code, item, user_id, date) VALUES (@code, @item, @user_id, @date)", {code = code, item = item, user_id = user_id, date = os.date("%d/%m/%Y")})
            else
                FRclient.notify(source, {"~r~You don not have that type of key!"})
            end
        end
    end)
end)
RegisterCommand("addkeys", function(source, args)
    if source == 0 then
        ...
        exports['fr']:execute('SELECT ?? FROM fr_crates WHERE user_id = ?', { key, user_id }, function(result)
            if result and result[1] then    
                local keys = result[1][key]
                keys = tonumber(keys) or 0
                keys = keys + amount
                exports['fr']:execute('UPDATE fr_crates SET ?? = ? WHERE user_id = ?', { key, keys, user_id })
                print("FR - Added " .. amount .. " " .. key .. " keys to user_id: " .. user_id)
            else
...
RegisterNetEvent("FR:setPlayerKeys")
AddEventHandler("FR:setPlayerKeys", function(target_id, key, amount)
    local source = source
    local user_id = FR.getUserId(source)
    if FR.hasGroup(user_id, "Founder") or FR.hasGroup(user_id, 'Developer') or FR.hasGroup(user_id, 'Lead Developer') then
        exports['fr']:execute('SELECT '.. key ..'  FROM fr_crates WHERE user_id = ?', { target_id }, function(result)
            if result and result[1] then    
                local keys = result[1][key]
                keys = tonumber(keys) or 0
                keys = keys + amount
                exports['fr']:execute('UPDATE fr_crates SET ?? = ? WHERE user_id = ?', { key, keys, target_id })
            end
        end)
    else
        TriggerEvent("FR:acBan", user_id, 11, FR.getPlayerName(source), source, 'FR:setPlayerKeys')
    end
end)

RegisterNetEvent("FR:getPlayerKeys")
AddEventHandler("FR:getPlayerKeys", function(target_id)
    local source = source
    local user_id = FR.getUserId(source)
    if FR.hasGroup(user_id, "Founder") or FR.hasGroup(user_id, 'Developer') or FR.hasGroup(user_id, 'Lead Developer') then
        exports['fr']:execute('SELECT * FROM fr_crates WHERE user_id = ?', { target_id }, function(result)
            if result and result[1] then    
                local keys = {}
                keys.silver = result[1].silver
                keys.gold = result[1].gold
                ...
                TriggerClientEvent("FR:Crate:GotPlayerKeys", source, keys)
            end
        end)
    else
        TriggerEvent("FR:acBan", user_id, 11, FR.getPlayerName(source), source, 'FR:getPlayerKeys')
    end
end)
...
Citizen.CreateThread(function()
    Wait(2500)
    exports['fr']:execute([[
        CREATE TABLE IF NOT EXISTS `fr_crates` (
