...
MySQL.createCommand("casinochips/remove_chips", "UPDATE fr_casino_chips SET chips = (chips - @amount) WHERE user_id = @user_id")
    ...
    if source ~= nil then
        ...
        if chairId > 7 then
            if FR.hasGroup(user_id,"Highroller") then
                ...
                FRclient.notify(source,{'You need to be a Highroller to sit at this table.'})
                return
            end
        end
        if chairId <= 7 then
                    ...
                    MySQL.query("casinochips/get_chips", {user_id = user_id}, function(rows, affected)
                        ...
                        if chips >= betAmount then
                            if chairId < 8 and betAmount > 100000 then
                                FRclient.notify(source,{'Maximum bet at this table is £100,000.'})
                                return
                            end
                            ...
                            FR.sendWebhook('blackjack-bet',"FR Blackjack Logs", "> Player Name: **"..FR.getPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Amount: **"..getMoneyStringFormatted(tostring(betAmount)).."**")
                        else 
                                            ...
                                            if blackjackGameData[gameId][source][2][nextCardCount] == true then --if hit 
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    if lostAmount > 10000000 then
                                                        TriggerClientEvent('chatMessage', -1, "^7Diamond Casino |", { 128, 128, 128 }, ""..FR.getPlayerName(source).." has LOST "..tostring(getMoneyStringFormatted(lostAmount)).." chips!", "alert")
                                                    end
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "^7Diamond Casino |", { 124, 252, 0 }, ""..FR.getPlayerName(source).." has WON "..tostring(getMoneyStringFormatted(potentialWinAmount)).." chips!", "green")
                                                            end
                                                        end
                                                    end
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "^7Diamond Casino |", { 128, 128, 128 }, ""..FR.getPlayerName(source).." has LOST "..tostring(getMoneyStringFormatted(potentialPushAmount)).." chips!", "alert")
                                                            end
                                                        end
                                                    end
                                                end
