...
function FR.getUserGroups(user_id)
...
function FR.addUserGroup(user_id, group)
    if not FR.hasGroup(user_id, group) then
        local user_groups = FR.getUserGroups(user_id)
        ...
        if ngroup then
            ...
            TriggerEvent("FR:playerJoinGroup", user_id, group, gtype)
        end
    end
end
...
function FR.getUserGroupByType(user_id, gtype)
    local user_groups = FR.getUserGroups(user_id)
    for k, v in pairs(user_groups) do
        local kgroup = groups[k]
        if kgroup then
...
function FR.getUsersByGroup(group)
    local users = {}

    for k, v in pairs(FR.rusers) do
        if FR.hasGroup(tonumber(k), group) then
...
function FR.getUsersByPermission(perm)
    local users = {}

    for k, v in pairs(FR.rusers) do
        if FR.hasPermission(tonumber(k), perm) then
            table.insert(users, tonumber(k))
        end
    end
...
function FR.removeUserGroup(user_id, group)
    local user_groups = FR.getUserGroups(user_id)
    ...
    TriggerEvent("FR:playerLeaveGroup", user_id, group, gtype)

    user_groups[group] = nil -- remove reference
end

-- check if the user has a specific group
function FR.hasGroup(user_id, group)
    local user_groups = FR.getUserGroups(user_id)
...
-- check if the user has a specific permission
function FR.hasPermission(user_id, perm)
    local user_groups = FR.getUserGroups(user_id)
    ...
    if fchar == "@" then -- special aptitude permission
        ...
        if #parts == 3 then -- decompose group.aptitude.operator
            ...
            local afr = math.floor(FR.expToLevel(FR.getExp(user_id, group, aptitude)))

            local fop = string.sub(op, 1, 1)
            if fop == "<" then -- less (group.aptitude.<x)
                local fr = parseInt(string.sub(op, 2, string.len(op)))
                if afr < fr then
                    return true
                end
            elseif fop == ">" then -- greater (group.aptitude.>x)
                ...
                local fr = parseInt(string.sub(op, 1, string.len(op)))
                if afr == fr then
                    return true
                end
            end
    ...
    elseif fchar == "#" then -- special item permission
        ...
        if #parts == 2 then -- decompose item.operator
            ...
            local amount = FR.getInventoryItemAmount(user_id, item)
...
function FR.hasPermissions(user_id, perms)
...
exports('hasPermission', function(source, perm)
    return FR.hasPermission(FR.getUserId(source), perm)
end)

exports('hasGroup', function(source, group)
    return FR.hasGroup(FR.getUserId(source), group)
end)