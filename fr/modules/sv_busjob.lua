...
RegisterNetEvent("FR:attemptBeginBusJob")
AddEventHandler("FR:attemptBeginBusJob",function()
    local source=source
    local user_id=FR.getUserId(source)
    local BusJobTable={}
    if FR.hasGroup(user_id,"Bus Driver")then
        if not usersInBusJob[user_id] then
            ...
            TriggerClientEvent("FR:beginBusJob",source)
            Wait(100)
            TriggerClientEvent("FR:setNextBusJobBlip",source,busJobStops[BusJobTable.stopNumber].stopPosition)
        else
            FRclient.notify(source,{"You are already a Bus Driver."})
        end
    else
        FRclient.notify(source,{"~r~You are not clocked on as a Bus Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(busJobStops)do
                    if FR.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(FR.getUserSource(k)))-busJobStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            FR.giveBankMoney(k,pay)
                            TriggerClientEvent("FR:nextStopReachedBusJob",FR.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#busJobStops then
                                TriggerClientEvent("FR:endBusJob", FR.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInBusJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("FR:setNextBusJobBlip",FR.getUserSource(k),busJobStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
