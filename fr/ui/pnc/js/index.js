...
async function RegisterVueComponent(component, path) {
    const response = await fetch("nui://fr/ui/pnc/components/"+path);
        ...
        methods: {
            addMarker() {
                if(!this.doesPlayerHaveWarningMarkerOfType(this.selectedWarningMarkerType)) {
                    $.post('https://fr/addmarker', JSON.stringify({
            ...
            wipeAllMarkers() {
                $.post('https://fr/wipeallmarkers', JSON.stringify({
            ...
            addPoint() {
                $.post('https://fr/addpoint', JSON.stringify({
            ...
            switchPage(page) {
                if(this.personPages.includes(page)){
                    if (this.selectedPerson.id){
                        ...
                        console.log("[FR PNC] Person needs to be selected to enter this page")
                    }
                }else {
                    this.page = page;
                }
            ...
            closePNC() {
                $.post('https://fr/exit', JSON.stringify({}));
            },
            searchPerson(){
                let first = this.personFirstName;
                let last = this.personLastName;
                if (first == "" || last == "") {
                    console.log("[FR PNC] Full name needs to be provided")
                }else{
                    $.post('https://fr/personsearch', JSON.stringify({
                        firstname: first,
                        lastname: last
                    }));
                }
            ...
            searchPlate(){
                ...
                $.post("https://fr/platesearch", JSON.stringify({
                    plate: this.plate
                }))
            },
            selectPerson(person){
            ...
            submitFine() {
                ...
                $.post('https://fr/submitfine', JSON.stringify({
            ...
            addAD(){
                ...
                if (this.ADInput.length === 0) {
                    ...
                    $.post('https://fr/addattentiondrawn', JSON.stringify({
                        ad: this.ADInput
                    }));
                }
            ...
            removeAD(index, ad) {
                if(this.askedIfSure) {
                    ...
                    $.post('https://fr/removeattentiondrawn', JSON.stringify({
            ...
            saveNotes() {
                $.post('https://fr/savenotes', JSON.stringify({
            ...
            savePersonNotes() {
                $.post("https://fr/savepersonnotes", JSON.stringify({
                    user_id: this.selectedPerson.id,
                    notes: this.selectedPerson.notes
                }))
            },
            GetWarrant() {
                $.post("https://fr/generatewarrant", JSON.stringify({}))
            }
            },
...
function clearAllPersonData(){
    ...
    pnc.selectedOldFine.suggestedAmount = 0;
