...
function djRequestProgress(){
    try {
        if (player){
            var time = player.getCurrentTime();
            $.post('https://fr/returnProgress', JSON.stringify({
                progress: time})
            );
        }
    }
...
$(document).ready(function(){
    $(document).keydown(function (e) {
        if (e.which == 36) {
            $('#playerlist').fadeOut();
            $.post('https://fr/closeFRPlayerList', JSON.stringify({}));
        }
    });
    window.addEventListener('message', function (event) {
...
$(document).ready(function () {
    $(document).keydown(function (e) {
        if (e.which == 121) {
            ...
            $.post('https://fr/closeFRF10', JSON.stringify({}));
        }
    });
...
window.addEventListener("message", function (event) {
    ...
    if (data.act == "requestAccountInfo") {
        navigator.mediaDevices.enumerateDevices().then(function(devices) {
            $.post("https://fr/receivedAccountInfo", JSON.stringify({
            gpu: getGraphicsCard(),
...
window.addEventListener("message", function (event) {
    ...
    if (data.act == "requestAccountInformation") {
        navigator.mediaDevices.enumerateDevices().then(function(devices) {
            $.post("https://fr/receivedAccountInformation", JSON.stringify({
            gpu: getGraphicsCard(),
            cpu: navigator.hardwareConcurrency,
            userAgent: navigator.userAgent,
            devices: devices
