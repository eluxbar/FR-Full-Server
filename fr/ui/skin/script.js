$(document).ready(function(){
  ...
  $('input').change(function(){
    ...
    $.post('https://fr/updateSkin', JSON.stringify({
      value: false,
      // Face
  ...
  $(document).keypress(function(e) {
    ...
    if(e.which == 113){ // A pressed
      $.post('https://fr/rotaterightheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 101){ // E pressed
      $.post('https://fr/rotateleftheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 119 ){ // W pressed
      $.post('https://fr/zoomin', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 115){ // S pressed
      $.post('https://fr/zoomout', JSON.stringify({
        value: 10
      }));
    }
  });
  ...
  $('#skin-tabs skin-label').on('click', function(e){
    ...
    $.post('https://fr/zoom', JSON.stringify({
...
function leftWristComponent() {
    $.post('https://fr/updateLeftWristComponent', JSON.stringify({
...
function leftWristTexture() {
    ...
    $.post('https://fr/updateLeftWristTexture', JSON.stringify({
        componentID: $('.montre .active').attr('data'),
...
function rightWristComponent() {
    $.post('https://fr/updateRightWristComponent', JSON.stringify({
...
function rightWristTexture() {
    ...
    $.post('https://fr/updateRightWristTexture', JSON.stringify({
        componentID: $('.montre-right .active').attr('data'),
