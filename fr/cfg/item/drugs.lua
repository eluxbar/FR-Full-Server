...
cocaine_sniff["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    if FR.tryGetInventoryItem(user_id,"Cocaine",1) then
      FRclient.notify(player,{"~g~Taking Cocaine."})
      TriggerEvent('FR:RefreshInventory', player)
      TriggerClientEvent('FR:cocaineEffect', player)
    end
  end
end}
...
heroin_take["Take"] = {function(player,choice)
  local user_id = FR.getUserId(player)
  if user_id ~= nil then
    if FR.tryGetInventoryItem(user_id,"Heroin",1) then
      FRclient.notify(player,{"~g~Taking Heroin."})
      TriggerEvent('FR:RefreshInventory', player)
      TriggerClientEvent('FR:heroinEffect', player)
    end
  end
end}
...
lsd_take["Take"] = {function(player,choice)
  local user_id = FR.getUserId(player)
  if user_id ~= nil then
    if FR.tryGetInventoryItem(user_id,"LSD",1) then
      FRclient.notify(player,{"~g~Taking LSD."})
      TriggerEvent('FR:RefreshInventory', player)
      TriggerClientEvent('FR:doAcid', player)
    end
  end
end}
...
morphine_choices["Take"] = {function(player,choice)
  local user_id = FR.getUserId(player)
  if user_id ~= nil then
    if FR.tryGetInventoryItem(user_id,"Morphine",1) then
      TriggerEvent('FR:RefreshInventory', player)
      TriggerClientEvent('FR:applyMorphine', player)
    end
  end
end}
...
taco_choices["Take"] = {function(player,choice)
  local user_id = FR.getUserId(player)
  if user_id ~= nil then
    if FR.tryGetInventoryItem(user_id,"Taco",1) then
      TriggerEvent('FR:RefreshInventory', player)
      TriggerClientEvent('FR:eatTaco', player)
    end
  end
end}
