...
-- it is not recommended to modify the FR core files outside the cfg/ directory, create a new resource instead
-- you can load external images/fonts/etc using the NUI absolute path: nui://my_resource/myfont.ttf
-- example, changing the gui font (suppose a fr_mod resource containing a custom font)
cfg.css = [[
@font-face {
  font-family: "Custom Font";
  src: url(nui://fr_mod/customfont.ttf) format("truetype");
}
...
-- static menus are menu with choices defined by FR.addStaticMenuChoices(name, choices)
