    ...
    if Menu ~= nil then
        if Menu() then
            if type(Value) == "boolean" then
                if noclipActive and tFR.getStaffLevel() < 4 and Menu.Sprite.Dictionary ~= "new_editor" then
                    return
                end
                if Value then
...
function RageUI.IsVisible(menu, header, glare, instructional, items, panels)
    if (RageUI.Visible(menu)) then
        ...
        if (items ~= nil) then
            ...
            if not success and not hasPrinted then
                hasPrinted = true
            end
        end
        ...
        if not ran and not hasPrinted then
            hasPrinted = true
        end
    end
end
