...
RegisterNetEvent(
    ...
    function()
        ...
        a.onJob = true
        tFR.notify("~g~Bus Job started, exit the bus station and head to the first bus stop.")
        while a.onJob do
...
RegisterNetEvent(
    ...
    function(d)
        ...
        local f = tFR.loadModel(b[a.stopNumber].pedModel)
        local g = tFR.getPosition()
        local h = 10.0
...
RegisterNetEvent(
    "FR:setNextBusJobBlip",
    function(n)
        a.tempBlip = AddBlipForCoord(n.x, n.y, n.z)
        SetBlipSprite(a.tempBlip, 1)
        SetBlipRoute(a.tempBlip, true)
        a.tempMarker = tFR.addMarker(n.x, n.y, n.z, 1.2, 1.2, 1.2, 255, 0, 0, 125, 50, 0, true, true)
    end
)
        ...
        if p then
            ...
            local s = function(r)
                if IsControlJustReleased(1, 38) and not a.onJob then
                    TriggerServerEvent("FR:attemptBeginBusJob")
                end
            end
            tFR.addBlip(a.startVector.x, a.startVector.y, a.startVector.z, 106, 1, "Bus Driver Job")
            tFR.addMarker(
                a.startVector.x,
                a.startVector.y,
                a.startVector.z - 1,
            ...
            tFR.createArea("bus", a.startVector, 1.5, 6, q, m, s, {})
        end
    end
)
function alert(t)
