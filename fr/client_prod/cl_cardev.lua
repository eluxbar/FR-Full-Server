RMenu.Add(
    "cardev",
    "mainmenu",
    RageUI.CreateMenu("", "", tFR.getRageUIMenuWidth(), tFR.getRageUIMenuHeight(), "fr_adminui", "fr_adminui")
)
    ...
    function()
        if RageUI.Visible(RMenu:Get("cardev", "mainmenu")) then
            RageUI.DrawContent(
                ...
                function()
                    if b and tFR.getPlayerBucket() == 333 then
                            ...
                            function(h, i, j)
                                if j then
                                    tFR.clientPrompt(
                                        ...
                                        "",
                                        function(k)
                                            if k ~= "" and tFR.getPlayerBucket() == 333 then
                                                local l = tFR.loadModel(k)
                                                TriggerServerEvent("FR:logVehicleSpawn", k, "/cardev")
                                                local m = tFR.getPlayerCoords()
                                                local n =
                                                    tFR.spawnVehicle(
                                                    l,
                                                    m.x,
                                                ...
                                                DecorSetInt(n, decor, 945)
                                                SetVehicleOnGroundProperly(n)
                                                SetEntityInvincible(n, false)
                                                SetPedIntoVehicle(tFR.getPlayerPed(), n, -1)
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(h, i, j)
                                if j then
                                    local o = GetVehiclePedIsIn(tFR.getPlayerPed(), false)
                        ...
                        RageUI.Checkbox(
                            ...
                            function()
                                b = true
                                TriggerServerEvent("FR:setCarDevMode", b)
                            end,
                            function()
                                b = false
                                TriggerServerEvent("FR:setCarDevMode", b)
            ...
            RageUI.DrawContent(
                ...
                function()
                    for w, x in pairs(f) do
                        if GetNumVehicleMods(tFR.getPlayerVehicle(), w) > 0 then
                            RageUI.ButtonWithStyle(
                                x,
                                "",
                                {RightLabel = "→→→"},
        ...
        if RageUI.Visible(RMenu:Get("cardev", "vehiclemodindexes")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    if GetNumVehicleMods(tFR.getPlayerVehicle(), selectedModType) == 0 then
                        RageUI.Text({message = "No available mod indexes for this mod type for this vehicle."})
                    else
                        for w = 0, GetNumVehicleMods(tFR.getPlayerVehicle(), selectedModType) do
                            RageUI.ButtonWithStyle(
                                "Mod " .. w,
                                "",
                                {RightLabel = "→→→"},
                                ...
                                function(h, i, j)
                                    if j then
                                        SetVehicleModKit(tFR.getPlayerVehicle(), 0)
                                        SetVehicleMod(tFR.getPlayerVehicle(), selectedModType, w)
                                    end
                                end,
                                RMenu:Get("cardev", "vehiclemodindexes")
                            )
        ...
        if RageUI.Visible(RMenu:Get("cardev", "extras")) then
            RageUI.DrawContent(
                ...
                function()
                                    ...
                                    RageUI.Button(
                                        ...
                                        function(h, i, j)
                                            if j then
                                                ...
                                                TriggerEvent("FR:staffFixVehicle")
                                            end
                                        end
                                    )
                                end
                    ...
                    if not y then
                        RageUI.Text({message = "No available extras for this vehicle."})
                    end
                end
            )
        end
                ...
                function()
                    for z, A in pairs(g) do
                        RageUI.Button(
                            ...
                            function(h, i, j)
                                if j then
                                    SetVehicleColours(tFR.getPlayerVehicle(), A.colorindex, A.colorindex)
                                end
                            end
                        )
                    end
    ...
    elseif V.type == "vector" then
        ...
        if a3 and #a3 >= 3 then
            ...
            if a4 and a5 and a6 then
                ...
                notify("Can not parse vector.")
            end
        else
            notify("Expected 3 floats.")
        end
    end
...
local function ae()
    if not b or tFR.getPlayerBucket() ~= 333 then
        if J then
            ac(false)
        end
        return
    ...
    L()
    local af = tFR.getFontId("Akrobat-ExtraBold")
    ...
    DrawAdvancedTextNoOutline(
        ...
        0.02,
        0.35,
        string.format("Top Speed: %.5f", K.speedDisplay),
        255,
        255,
        255,
        255,
    ...
    DrawAdvancedTextNoOutline(
        ...
        0.02,
        0.35,
        string.format("Top Acceleration: %.5f", K.accelDisplay),
        255,
        255,
        255,
        255,
    ...
    DrawAdvancedTextNoOutline(
        ...
        0.02,
        0.35,
        string.format("Top Deacceleration: %.5f", K.decelDisplay),
        255,
        255,
        255,
        255,
    ...
    for al, am in pairs(F) do
        ...
        if
            CursorInArea(0.2505 + ai, 0.3655 + ai, 0.665 + ah * 0.05, 0.705 + ah * 0.05) and
