...
local o = json.decode(GetResourceKvpString("fr_gang_inv_colour")) or {r = 0, g = 50, b = 142}
local p = nil
local q = nil
local r = nil
local s = false
...
local t = false
local function u()
    if IsUsingKeyboard(2) and not tFR.isInComa() and not tFR.isHandcuffed() then
        ...
        if not i then
            ...
            if drawInventoryUI then
                ...
                inGUIFR = false
                if p then
                    tFR.vc_closeDoor(q, 5)
                    p = nil
                    q = nil
                    r = nil
                    TriggerEvent("FR:clCloseTrunk")
                end
                inventoryType = nil
                FRSecondItemList = {}
            end
        else
            tFR.notify("~r~Cannot open inventory right before a restart!")
        end
    end
end
...
local x = 0
local y = 14
function tFR.getSpaceInFirstChest()
    return currentInventoryMaxWeight - d
end
function tFR.getSpaceInSecondChest()
    local z = 0
...
RegisterNetEvent(
    ...
    function(x, y, D, E)
        ...
        if D then
            g = D
            h = GetEntityCoords(tFR.getPlayerPed())
            if D == "notmytrunk" then
                j = GetEntityCoords(tFR.getPlayerPed())
            end
...
AddEventHandler(
    "FR:clOpenTrunk",
    function()
        local F, G, H = tFR.getNearestOwnedVehicle(3.5)
        r = G
        q = H
        if F and IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
            ...
            tFR.vc_openDoor(G, 5)
            inventoryType = "CarBoot"
            TriggerServerEvent("FR:FetchTrunkInventory", G)
        else
            tFR.notify("~r~You don't have the keys to this vehicle!")
        end
    end
)
Citizen.CreateThread(
    function()
        while true do
            if f ~= nil and c then
                ...
                if K > 10.0 then
                    TriggerEvent("FR:clCloseTrunk")
                    TriggerServerEvent("FR:closeChest")
                end
            end
        ...
        while true do
            if drawInventoryUI then
                ...
                DrawAdvancedText(
                    ...
                    255,
                    255,
                    tFR.getFontId("Akrobat-ExtraBold"),
                    0
                )
                ...
                if m then
                    if CursorInArea(0.46, 0.496, 0.33, 0.383) then
                        ...
                        if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                            ...
                            if not lockInventorySoUserNoSpam then
                                if a then
                                    TriggerServerEvent("FR:UseItem", a, "Plr")
                                elseif b and g ~= nil and c then
                                    FRserver.useInventoryItem({b})
                                else
                                    tFR.notify("~r~No item selected!")
                                end
                            end
                            lockInventorySoUserNoSpam = true
                            Citizen.CreateThread(
                    ...
                    if CursorInArea(0.501, 0.536, 0.329, 0.381) then
                        ...
                        if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                            ...
                            if not lockInventorySoUserNoSpam then
                                if a then
                                    TriggerServerEvent("FR:UseAllItem", a, "Plr")
                                elseif b and g ~= nil and c then
                                    FRserver.useInventoryItem({b})
                                else
                                    tFR.notify("~r~No item selected!")
                ...
                if not m then
                    if CursorInArea(0.4598, 0.5333, 0.5931, 0.6477) then
                        ...
                        if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                            ...
                            if not lockInventorySoUserNoSpam then
                                if a then
                                    TriggerServerEvent("FR:GiveItem", a, "Plr")
                                elseif b then
                                ...
                                if a and g ~= nil and c then
                                    ...
                                    if tFR.getPlayerCombatTimer() > 0 then
                                        ...
                                        if inventoryType == "CarBoot" then
                                            TriggerServerEvent("FR:MoveItem", "Plr", a, r, false)
                                        elseif inventoryType == "Housing" then
                                            TriggerServerEvent("FR:MoveItem", "Plr", a, "home", false)
                                        elseif inventoryType == "Crate" then
                                            TriggerServerEvent("FR:MoveItem", "Plr", a, "crate", false)
                                        elseif s then
                                            TriggerServerEvent("FR:MoveItem", "Plr", a, "LootBag", true)
                                        end
                                    end
                                elseif b and g ~= nil and c then
                                    if inventoryType == "CarBoot" then
                                        TriggerServerEvent("FR:MoveItem", inventoryType, b, r, false)
                                    elseif inventoryType == "Housing" then
                                        TriggerServerEvent("FR:MoveItem", inventoryType, b, "home", false)
                                    elseif inventoryType == "Crate" then
                                        TriggerServerEvent("FR:MoveItem", inventoryType, b, "crate", false)
                                    else
                                        TriggerServerEvent("FR:MoveItem", "LootBag", b, LootBagIDNew, true)
                                    end
                                else
                                    tFR.notify("~r~No item selected!")
                                end
                            else
                                tFR.notify("~r~No second inventory available!")
                            end
                        end
                        lockInventorySoUserNoSpam = true
                        Citizen.CreateThread(
                            ...
                            if c then
                                if a and g ~= nil and c then
                                    ...
                                    if tFR.getPlayerCombatTimer() > 0 then
                                        ...
                                        if inventoryType == "CarBoot" then
                                            TriggerServerEvent("FR:MoveItemX", "Plr", a, r, false, a4)
                                        elseif inventoryType == "Housing" then
                                            TriggerServerEvent("FR:MoveItemX", "Plr", a, "home", false, a4)
                                        elseif inventoryType == "Crate" then
                                            TriggerServerEvent("FR:MoveItemX", "Plr", a, "crate", false, a4)
                                        elseif s then
                                            TriggerServerEvent("FR:MoveItemX", "Plr", a, "LootBag", true, a4)
                                        end
                                    end
                                elseif b and g ~= nil and c then
                                    if inventoryType == "CarBoot" then
                                        TriggerServerEvent("FR:MoveItemX", inventoryType, b, r, false, a4)
                                    elseif inventoryType == "Housing" then
                                        TriggerServerEvent("FR:MoveItemX", inventoryType, b, "home", false, a4)
                                    elseif inventoryType == "Crate" then
                                        TriggerServerEvent("FR:MoveItemX", inventoryType, b, "crate", false, a4)
                                    else
                                        TriggerServerEvent("FR:MoveItemX", "LootBag", b, LootBagIDNew, true, a4)
                                    end
                                else
                                    tFR.notify("~r~No item selected!")
                                end
                ...
                if CursorInArea(0.5004, 0.5333, 0.5042, 0.5666) then
                    ...
                    if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                        ...
                        if not lockInventorySoUserNoSpam then
                            if c then
                                if a and g ~= nil and c then
                                    local L = tFR.getSpaceInSecondChest()
                                    local a4 = k
                                        ...
                                        if tFR.getPlayerCombatTimer() > 0 then
                                            ...
                                            if inventoryType == "CarBoot" then
                                                TriggerServerEvent(
                                                    ...
                                                    a,
                                                    r,
                                                    NetworkGetNetworkIdFromEntity(tFR.getNearestVehicle(3))
                                                )
                                            elseif inventoryType == "Housing" then
                                                TriggerServerEvent("FR:MoveItemAll", "Plr", a, "home")
                                            elseif inventoryType == "Crate" then
                                                TriggerServerEvent("FR:MoveItemAll", "Plr", a, "crate")
                                            elseif s then
                                                TriggerServerEvent("FR:MoveItemAll", "Plr", a, "LootBag")
                                            end
                                        end
                                    else
                                        tFR.notify("~r~Not enough space in secondary chest!")
                                    end
                                elseif b and g ~= nil and c then
                                    local M = tFR.getSpaceInFirstChest()
                                    local a4 = k
                                    ...
                                    if a4 > 0 then
                                        ...
                                        elseif inventoryType == "Housing" then
                                            TriggerServerEvent("FR:MoveItemAll", inventoryType, b, "home")
                                        elseif inventoryType == "Crate" then
                                            TriggerServerEvent("FR:MoveItemAll", inventoryType, b, "crate")
                                        else
                                            TriggerServerEvent("FR:MoveItemAll", "LootBag", b, LootBagIDNew)
                                        end
                                    else
                                        tFR.notify("~r~Not enough space in secondary chest!")
                                    end
                ...
                if m then
                    if CursorInArea(0.5004, 0.5333, 0.5931, 0.6477) then
                        ...
                        if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                            ...
                            if not lockInventorySoUserNoSpam then
                                if a then
                                    TriggerServerEvent("FR:GiveItemAll", a, "Plr")
                                elseif b then
                                    FRserver.giveToNearestPlayer({b})
                                else
                                    tFR.notify("~r~No item selected!")
                ...
                if s then
                    if CursorInArea(0.5428, 0.5952, 0.2879, 0.3111) then
                        ...
                        if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            if not lockInventorySoUserNoSpam then
                                TriggerServerEvent("FR:LootItemAll", LootBagIDNew)
                            end
                            lockInventorySoUserNoSpam = true
                            Citizen.CreateThread(
                                function()
                ...
                if next(v) then
                    if CursorInArea(0.233854, 0.282813, 0.287037, 0.308333) then
                        ...
                        if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            if not lockInventorySoUserNoSpam then
                                TriggerServerEvent("FR:EquipAll")
                            end
                            lockInventorySoUserNoSpam = true
                            Citizen.CreateThread(
                                function()
                ...
                if CursorInArea(0.4598, 0.5333, 0.6831, 0.7377) then
                    ...
                    if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                        ...
                        if not lockInventorySoUserNoSpam then
                            if a then
                                TriggerServerEvent("FR:TrashItem", a, "Plr")
                            elseif b then
                                tFR.notify("~r~Please move the item to your inventory to trash")
                            else
                                tFR.notify("~r~No item selected!")
                            end
                        end
        ...
        while true do
            ...
            if drawInventoryUI then
                if
                 ...
                 then
                    TriggerEvent("FR:InventoryOpen", false)
                    if p then
                        tFR.vc_closeDoor(q, 5)
                        p = nil
                        q = nil
...
RegisterNetEvent("FR:InventoryOpen")
AddEventHandler(
    "FR:InventoryOpen",
    function(a7, a8, a9)
        s = a8
...
function tFR.setInventoryColour()
    tFR.clientPrompt(
        ...
        function(aa)
            if aa ~= "" then
                ...
                if I[1] ~= nil and I[2] ~= nil and I[3] ~= nil then
                    ...
                    o.g = tonumber(I[2])
                    o.b = tonumber(I[3])
                    tFR.notify("~g~Inventory colour updated.")
                else
            ...
            SetResourceKvp("fr_gang_inv_colour", json.encode(o))
        end
    )
end
