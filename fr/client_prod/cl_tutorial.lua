...
local function r(s, t)
    ...
    while #(tFR.getPlayerCoords() - s) > 4.0 do
        Citizen.Wait(0)
    end
    RemoveBlip(v)
    DeleteCheckpoint(u)
...
local function w(x, y, z)
    tFR.loadModel("metrotrain")
    tFR.loadModel("s_m_m_lsmetro_01")
    local A = 0
    while true do
...
local function N()
    TriggerEvent("FR:Announce", "FOLLOW THE YELLOW MARKERS!")
...
local function O()
    local P = tFR.addMarker(b.x, b.y, b.z, 1.0, 1.0, 1.0, 50, 205, 50, 150, 50, 32, false, false, true)
    ...
    ScaleformMovieMethodAddParamTextureNameString("FR TUTORIAL")
    ScaleformMovieMethodAddParamTextureNameString("~g~Welcome to FR")
    ...
    tFR.loadAnimDict("cellphone@")
    tFR.loadModel("prop_amb_phone")
    p("ring")
    Citizen.Wait(2000)
    local H = tFR.getPlayerCoords()
...
local function V()
    ...
    while tFR.getPlayerVehicle() ~= l do
        Citizen.Wait(0)
    end
    h = nil
    DeleteCheckpoint(u)
...
local function X(Y, Z, _)
    ...
    while #(tFR.getPlayerCoords() - Y) > a0 do
        if _ then
            _()
        end
        Citizen.Wait(0)
...
local function a1()
    ...
    while tFR.getPlayerVehicle() ~= 0 do
        Citizen.Wait(0)
    end
    ...
    while table.count(VehiclesFetchedTable) == 0 do
        ...
        elseif DoesEntityExist(testDriveCar) then
            ...
            local a2 = #(vector3(-34.113563537598, -1101.7242431641, 25.422456741333) - tFR.getPlayerCoords()) <= 2.5
            if a2 then
...
local function a3()
    ...
    while table.count(globalVehicleModelHashMapping) == 0 or tFR.getPlayerVehicle() == 0 do
        if RageUI.Visible(RMenu:Get("FRGarages", "main")) then
            ...
            h = "Select Garages"
        elseif RageUI.Visible(RMenu:Get("FRGarages", "owned_vehicles")) then
            i = "This lists all your garage types. For now you will only have the Standard Garage."
            h = "Select Standard Garage"
        elseif RageUI.Visible(RMenu:Get("FRGarages", "owned_vehicles_submenu")) then
            i = "This lists all the vehicles you have bought for this garage type."
            h = "Select your newly purchased vehicle"
        elseif RageUI.Visible(RMenu:Get("FRGarages", "owned_vehicles_submenu_manage")) then
            i = "You can spawn your vehicle, or choose to sell and rent it to another player here."
            h = "Press Spawn Vehicle"
        elseif RageUI.Visible(RMenu:Get("FRGarages", "settings")) then
            i = nil
            h = "Go back to the main menu"
        elseif
            RageUI.Visible(RMenu:Get("FRGarages", "rented_vehicles")) or
                RageUI.Visible(RMenu:Get("FRGarages", "customfolders"))
         then
            RageUI.Visible(RMenu:Get("FRGarages", "main"), true)
            notify("~r~This section is not avaliable during the tutorial")
        else
            local a2 = #(vector3(-51.893981933594, -1112.7712402344, 25.438014984131) - tFR.getPlayerCoords()) < 2.5
            if a2 then
    ...
    Citizen.Wait(1000)
    SetEntityInvincible(tFR.getPlayerVehicle(), true)
    i = nil
    h = nil
end
local function a6()
    ...
    local a7 = tFR.getCustomization()
    local a8 = false
    while true do
        if not RageUI.IsAnyMenuOfTypeVisible("FRclothing") then
            if a8 then
                break
            end
            local a2 = #(vector3(123.60697174072, -219.52917480469, 53.557830810547) - tFR.getPlayerCoords()) < 2.5
            if a2 then
            ...
            elseif RageUI.Visible(RMenu:Get("FRclothing", "clearProps")) then
                i = "This can be used to remove accessories. You can also toggle them through the Y menu."
                h = "Go back to pick an outfit"
            end
            if not table.contentEquals(tFR.getCustomization(), a7) then
                a8 = true
            end
        end
        Citizen.Wait(0)
...
local function ac()
    ...
    EndScaleformMovieMethod()
    p("questcomplete")
    TriggerServerEvent("FR:setCompletedTutorial")
    local U = GetGameTimer()
...
local function ad()
    ...
    D()
    V()
    if table.count(VehiclesFetchedTable) == 0 or tFR.isDev() then
        if m > 20000 then
            a1()
        end
    else
...
RegisterNetEvent("FR:playTutorial", ad)
AddEventHandler(
    "FR:onClientSpawn",
    function(ae, af)
        if af and not tFR.isPurge() then
            Citizen.Wait(10000)
            TriggerServerEvent("FR:checkTutorial")
        end
    end
)
local function ag()
    if g then
        ...
        if not tFR.isPurge() then
            ...
            tFR.setWeapon(E, "WEAPON_UNARMED", true)
        end
    end
end
tFR.createThreadOnTick(ag)
RegisterNetEvent(
    "FR:setIsNewPlayer",
    function()
        f = true
...
function tFR.isNewPlayer()
    return f
end
function tFR.isInTutorial()
    return g
end


function tFR.OpenTutorialMenu()
    if tFR.isInTutorial() then
        FreezeEntityPosition(PlayerPedId(), true)
        tFR.setCustomization(getDefaultCustomization(false), true, true)
        SendNUIMessage({type = "OpenTutorialMenu", value = true})
        tFR.hideUI()
...
RegisterNUICallback(
    "PickGender",
    function(au)
        tFR.setCustomization(getDefaultCustomization(tobool(au.gender)), true, true)
        tFR.showUI()
