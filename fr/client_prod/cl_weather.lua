...
function tFR.cancelOverrideTimeWeather()
    c = false
end
function tFR.setWeather(i)
    a = i
end
...
tFR.createThreadOnTick(func_manageTimeAndWeather)
RegisterNetEvent(
    "FR:setTime",
    function(f, g, h)
        tFR.setTime(f, g, h)
...
function voteWeather(C)
    if j then
        ...
        tFR.notify("Vote not in progress, start a vote with /voteweather!")
    end
end
function resetVotes()
    m = {
...
RegisterNetEvent(
    ...
    function()
        ...
        TriggerEvent(
            "chatMessage",
            "Weather vote has started! Type /[weather] e.g /snow or /rain to vote.",
            {0, 250, 50}
        )
...
RegisterCommand(
    "voteweather",
    function()
        TriggerServerEvent("FR:tryStartWeatherVote")
    end,
    false
)
RegisterCommand(
