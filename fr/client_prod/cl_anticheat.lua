local a = module("fr-assets", "cfg/cfg_anticheat")
decor = nil
local function b(c, ...)
...
Citizen.CreateThread(
    function()
        Wait(15000)
        while true do
            local f = tFR.getPlayerPed()
            local g = tFR.getPlayerId()
            local h = tFR.getPlayerVehicle()
            if h == 0 then
                ...
                if not e and GetUsingseethrough() and not tFR.isPlayerInPoliceHeli() and not tFR.isPlayerInDrone() then
                    TriggerServerEvent("FR:acType13")
                    e = true
                end
            end
            SetPedInfiniteAmmoClip(f, false)
            for b, j in pairs(tFR.getWeapons()) do
...
function CheckVariables()
    for p, q in pairs(o) do
        if _G[q] ~= nil then
            TriggerServerEvent("FR:acType7", q)
        end
    end
end
Citizen.CreateThread(
...
Citizen.CreateThread(
    function()
        while true do
            ...
            if t > 1.0 then
                TriggerServerEvent("FR:acType8", "PlayerWeaponDamageModifier", t)
                return
            end
            if u > 1.0 then
                TriggerServerEvent("FR:acType8", "PlayerWeaponDefenseModifier", u)
                return
            end
            if v > 1.0 then
                TriggerServerEvent("FR:acType8", "PlayerWeaponDefenseModifier_2", v)
                return
            end
            if w > 1.0 then
                TriggerServerEvent("FR:acType8", "PlayerVehicleDamageModifier", w)
                return
            end
            if x > 1.0 then
                TriggerServerEvent("FR:acType8", "PlayerVehicleDefenseModifier", x)
                return
            end
            if A > 1.0 then
                TriggerServerEvent("FR:acType8", "GetWeaponDamageModifier", A)
                return
            end
            if B > 1.0 then
                TriggerServerEvent("FR:acType8", "GetPlayerMeleeWeaponDamageModifier", B)
                return
            end
...
function tFR.isPlayerAboveGround()
    local C = tFR.getPlayerCoords()
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            P = Q
            if
                R > 0.4 and not IsPedFalling(i) and tFR.getStaffLevel() < 2 and not IsPedInParachuteFreeFall(i) and
                    not carryingBackInProgress and
                    not piggyBackInProgress and
                    not tFR.takeHostageInProgress() and
                    GetPedParachuteState(i) <= 0 and
                    not IsPedRagdoll(i) and
                    not IsPedRunning(i) and
                    not tFR.isPlayerRappeling() and
                    not tFR.isPlayerAboveGround() and
                    not tFR.isPlayerHidingInBoot() and
                    not tFR.isSpectatingEvent() and
                    not noclipActive
             then
                if not IsPedInAnyVehicle(i, 1) then
                    F = F + 1
                    if F > 100 then
                        TriggerServerEvent("FR:acType1", false)
                        F = 0
                    end
                end
            end
            local f, S = tFR.getPlayerVehicle()
            if DoesEntityExist(f) and S and R > 0.2 and tFR.getStaffLevel() < 2 then
                if H ~= f then
                    G = 0
                    H = f
                end
                ...
                if L < 5.0 and T < 2.5 then
                    ...
                    if G > 100 and GetGameTimer() - I > 4000 then
                        TriggerServerEvent("FR:AnticheatBan", 1, true)
                        G = 0
                        I = GetGameTimer()
                    end
                end
            end
            if NetworkIsInSpectatorMode() and not tFR.isInSpectate() then
                TriggerServerEvent("FR:acType23")
                return
            end
            if
                GetCamFov(GetRenderingCam()) == 50.0 and not tFR.isInSpectate() and not tFR.isInTutorial() and
                    tFR.getStaffLevel() == 0
             then
                TriggerServerEvent("FR:acType24")
                Wait(10000)
            end
            Wait(0)
        end
        ...
        while true do
            ...
            for U, V in ipairs(WeaponBL) do
                ...
                if HasPedGotWeapon(PlayerPedId(), GetHashKey(V), false) == 1 then
                    ...
                    TriggerServerEvent("FR:acType2", V)
                end
            end
            local U, t = StatGetInt(GetHashKey("mp0_shooting_ability"), true)
            if t > 100 then
                TriggerServerEvent("FR:acType19")
            end
        end
    end
)
Citizen.CreateThread(
    function()
        ...
        local U = 0
        while true do
            if U >= 100 and not tFR.isInComa() then
                TriggerServerEvent("FR:acType6")
                return
            end
            if
                not tFR.isStaffedOn() and GetEntityHealth(PlayerPedId()) > 102 and not spawning and
                    not tFR.isPedScriptGuidChanging()
             then
                local g = PlayerId()
                local f = PlayerPedId()
                ...
                if f ~= 0 then
                    ...
                    tFR.setHealth(GetEntityHealth(f) + 2)
                end
            else
                Citizen.Wait(1000)
            end
...
local function aw()
    ...
    local aA = #(az - a1)
    if
        aA < 50.0 or ay or carryingBackInProgress or piggyBackInProgress or tFR.isPlayerHidingInBoot() or
    ...
    if at(aE, 100.0) then
        if ax - a5 > 5000 then
            TriggerServerEvent("FR:sendVelocityLimit", az, a1)
            a5 = ax
        end
    end
end
RegisterNetEvent(
    "FR:settingPlayerIntoVehicle",
    function()
        a4 = GetGameTimer()
    end
)
...
local function b2()
    local c, S = tFR.getPlayerVehicle()
    if
    ...
    if aZ(b3, aF) or aZ(b4, aG) or aZ(b5, aH) or aZ(b6, aI) then
        ...
        if
         ...
         then
            local aT = aS(c)
            local b7 = tFR.getVehicleIdFromHash(GetEntityModel(c)) or "N/A"
            TriggerServerEvent("FR:sendVehicleStats", b3, aF, b4, aG, b5, aH, b6, aI, aT, b7)
            aL = ax
        end
    end
    aF = b3
        ...
        while true do
            ...
            if GetVehicleHasParachute(c) or GetCanVehicleJump(c) or GetHasRocketBoost(c) and b9 ~= GetHashKey("voltic2") then
                if not table.has(a.anticheat, b9) then
                    TriggerServerEvent("FR:acType12", globalVehicleModelHashMapping[b9])
                    return
                end
            end
            Wait(1000)
...
Citizen.CreateThread(
    function()
        ...
        while true do
            if
                GetPlayerInvincible(PlayerId()) and not isInGreenzone and not tFR.isInsideLsCustoms() and
                    not tFR.isStaffedOn() and
                    not noclipActive and
                    not tFR.isInComa() and
                    not tFR.isInSpectate() and
                    not tFR.getInRPZone() and
                    not tFR.isInTutorial()
             then
                TriggerServerEvent("FR:acType15")
                Citizen.Wait(60000)
            end
            if
                GetPlayerInvincible_2(PlayerId()) and not isInGreenzone and not tFR.isInsideLsCustoms() and
                    not tFR.isStaffedOn() and
                    not noclipActive and
                    not tFR.isInComa() and
                    not tFR.isInSpectate() and
            ...
            if
                not IsEntityVisible(PlayerPedId()) and not tFR.isInsideLsCustoms() and not noclipActive and
                    not tFR.isInSpectate() and
                    not tFR.isInTutorial() and
                    not tFR.inEvent() and
                    not tFR.isPlayerInDrone() and
                    not tFR.isSpectatingEvent() and
                    not spawning and
                    tFR.getStaffLevel() < 3 and
                    GetEntityAttachedTo(PlayerPedId()) == 0
             then
                TriggerServerEvent("FR:acType27")
                Citizen.Wait(60000)
            end
            Citizen.Wait(1000)
        end
    ...
    function()
        while true do
            ...
            for a0, be in pairs(bd) do
                if bc == be then
                    TriggerServerEvent("FR:acType8", "WeaponDamageType", bc)
                    return
                end
            end
            Wait(0)
...
RegisterNUICallback(
    "NuiDevTool",
    function()
        TriggerServerEvent("FR:acType26")
    end
)
AddEventHandler(
    "onClientResourceStop",
    function(bf)
        TriggerServerEvent("FR:acType18", bf)
    end
)
AddEventHandler(
    "onResourceStop",
...
print("[FR] Anti-Cheat initialised - Credits * ")
