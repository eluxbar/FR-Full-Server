local a = module("fr-assets", "cfg/cfg_vehiclemaxspeeds")
isInGreenzone = false
local b = true
local c = false
local d = false
...
local n = {
    ...
    {vector3(1913.4696044922,4938.1791992188,77.610389709473), 100.0, 1, 180},
...
function tFR.areGreenzonesDisabled()
    return j
end
function tFR.setGreenzonesDisabled(q)
    j = q
end
o(
    function()
        for r, s in pairs(n) do
            if not s[5] and not tFR.isPurge() or s[5] and tFR.isPurge() then
                Wait(500)
...
o(
    function()
        local u = tFR.isPurge()
        while true do
            local v = tFR.getPlayerPed()
            local w = tFR.getPlayerCoords()
            for x, y in pairs(m) do
                if not y.isPurge and not u or y.isPurge and u then
                    ...
                    SetPlayerInvincible(tFR.getPlayerId(), false)
                    ...
                    SetNetworkVehicleAsGhost(tFR.getPlayerVehicle(), false)
                end
            end
            p(250)
        end
...
local function G()
    ...
    if I then
        local M, N = tFR.getPlayerVehicle()
        if M ~= 0 then
        ...
        while true do
            ...
            if not tFR.areGreenzonesDisabled() then
                ...
                if c or d then
                    ...
                    ClearPedBloodDamage(v)
                    if usingDelgun then
                        tFR.setWeapon(v, "WEAPON_STAFFGUN", true)
                    else
                        tFR.setWeapon(v, "WEAPON_UNARMED", true)
                    end
                    ResetPedVisibleDamage(v)
                    ClearPedLastWeaponDamage(v)
                ...
                if c then
                    ...
                    local W, X = tFR.getPlayerCombatTimer()
                    if W > 0 and X then
                        G()
                    end
                end
...
RegisterCommand(
    "togglecitycap",
    function()
        if tFR.getUserId() == 1 or tFR.getUserId() == 0 then
            b = not b
            tFR.notify("City Cap: " .. (b and "~g~Enabled" or "~r~Disabled"))
        end
    end
)
