    ...
    function()
        while true do
            if m and tFR.getPlayerVehicle() ~= k.limitingVehicle then
                ...
                predictedMax = nil
                tFR.notify("~r~Vehicle Changed, stopping limiter")
            end
            Wait(500)
        end
    end
    ...
    Citizen.CreateThread(
        function()
            while k.autoDrive and u do
                speed = k.adSpeed
                mode = j[k.adMode]
                local v = GetVehiclePedIsIn(tFR.getPlayerPed(), false)
                if v ~= 0 then
                    if IsWaypointActive() then
                        if not l or speed ~= q or mode ~= nil then
                            ...
                            local q = speed
                            local w = mode
                            TaskVehicleDriveToCoord(tFR.getPlayerPed(),v,
                                waypoint.x,
                                waypoint.y,
                                waypoint.z,
                                convert(speed),
                        ...
                        tFR.notify("~r~You do not have a waypoint set")
                        k.autoDrive = false
                    end
                else
                    tFR.notify("~r~You are not in a vehicle!")
                    k.autoDrive = false
                end
                local x = GetIsVehicleEngineRunning(v)
                local inWater = IsEntityInWater(v) and not IsPedInAnyBoat(tFR.getPlayerPed())
                u = x and not inWater
                if not x then
                    tFR.notify("~r~AutoDrive automatically disabled because the engine is inactive")
                    k.autoDrive = false
                elseif inWater then
                    tFR.notify("~r~AutoDrive automatically disabled because you are in water")
                    k.autoDrive = false
                end
                Wait(100)
            end
...
function cruise()
    ...
    Citizen.CreateThread(
        function()
            cruiseVehicle = GetVehiclePedIsUsing(tFR.getPlayerPed())
            local y = GetEntitySpeed(cruiseVehicle)
            local u = true
            while k.cruise and IsPedInAnyVehicle(tFR.getPlayerPed(), false) and u do
                while not accelerating and k.cruise and u do
                    ...
                    inWater = IsEntityInWater(cruiseVehicle) and not IsPedInAnyBoat(tFR.getPlayerPed())
            ...
            if collision then
                tFR.notify("~r~Cruise control automatically disabled because a collision was detected")
            elseif not engineStatus then
                tFR.notify("~r~Cruise control automatically disabled because the engine was turned off")
            elseif inWater then
                tFR.notify("~r~Cruise control automatically disabled because you are in water")
            end
            n = false
            k.cruise = false
        end
...
function acceleratingToNewSpeed(cruiseVehicle)
    while IsControlPressed(1, 71) or IsControlPressed(1, 72) or IsControlPressed(1, 76) or
        GetVehicleCurrentGear(cruiseVehicle) == 0 and IsPedInAnyVehicle(tFR.getPlayerPed(), false) do
        Wait(100)
    end
    ...
    accelerating = false
    local cruiseVehicle = GetVehiclePedIsUsing(tFR.getPlayerPed())
    ...
    function()
        RageUI.IsVisible(
            ...
            function()
                        ...
                        if K then
                            ...
                            if L ~= 0 then
                                ...
                                if M == 9 then
                                    ...
                                    tFR.notify("~r~Closed ~w~all doors.")
                                elseif M == 8 then
                                    if r then
                                        r = false
                                        RollDownWindow(L, 0)
                                ...
                                elseif IsVehicleDoorDamaged(L, N) then
                                    tFR.notify("~r~Cannot shut this door when the door is damaged")
                            ...
                            if k.limiter then
                                ...
                                if not p then
                                    ...
                                    if convert(speed) < 10.0 then
                                        ...
                                        p = false
                                        tFR.notify("~r~Alert~w~: Please slow down to enable the speed limiter.")
                                    end
                                end
                            else
                ...
                local isthere, spawncode, distance = tFR.getNearestOwnedVehicle(1)
                RageUI.Button(
                    "Manage Boot Keys",
                    ...
                    function(I, J, K)
                        if K then
                            TriggerServerEvent("FR:VehicleBoot:Check", spawncode)
                        end
                    end,
        ...
        RageUI.IsVisible(
            ...
            true,
            function()
                local _, spawncode = tFR.getNearestOwnedVehicle(1)
                local hasboot = true
                local boottable = {
                    {user_id = 1, name = "Envy"},
                ...
                if next(boottable) then
                    for _, v in pairs(boottable) do
                        RageUI.Button(
                            ...
                            function(I, J, K)
                                if K then
                                    TriggerServerEvent("FR:VehicleBoot:Remove", spawncode, v.user_id)
                                end
                            end
                        )
                    end
                ...
                RageUI.Button(
                    ...
                    function(I, J, K)
                        if K then
                            TriggerServerEvent("FR:VehicleBoot:Add", spawncode)
                        end
                    end
                )
                if not hasboot then
        ...
        RageUI.IsVisible(
            ...
            function()
                ...
                RageUI.Button(
                    ...
                    function(I, J, K)
                        if K then
                            TriggerServerEvent("FR:VehicleBoot:Purchase", spawncode)
                            if hasboot then
                                RageUI.GoBack()
                            end
                        end
...
RegisterNetEvent("FR:VehicleBoot:Return")
AddEventHandler(
    "FR:VehicleBoot:Return",
