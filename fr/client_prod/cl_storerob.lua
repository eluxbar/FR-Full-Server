...
AddEventHandler(
    ...
    function(t, u)
        if u then
            TriggerServerEvent("FR:getStoreRobBlips")
            while p == nil do
                Wait(0)
            end
            while true do
    ...
    if not IsPedDeadOrDying(x) then
        if IsEntityPlayingAnim(x, "mp_am_hold_up", "holdup_victim_20s", 3) then
            ...
            if not IsPedDeadOrDying(x) then
                ...
                local A = tFR.loadModel("prop_poly_bag_01")
                        ...
                        function()
                            while true do
                                ...
                                if DoesEntityExist(B) then
                                    if #(GetEntityCoords(tFR.getPlayerPed()) - GetEntityCoords(B)) <= 1.5 then
                                        PlaySoundFrontend(
                                            -1,
                                            "ROBBERY_MONEY_TOTAL",
                                        ...
                                        TriggerServerEvent("FR:pickupCashFromApu", v)
                                        DeleteObject(B)
                                        break
                                    end
                                else
...
Citizen.CreateThread(
    function()
        while true do
            local t = GetEntityCoords(tFR.getPlayerPed(), true)
            if r ~= nil then
                for v, w in pairs(r) do
...
function refreshStoreBlips(u)
    ...
    for D, E in pairs(p) do
        ...
        if E.beingrobbed == true then
            tempBlip = tFR.addBlip(F.x, F.y, F.z, 52, 1, "Robbable Store [BEING ROBBED]")
        else
            tempBlip = tFR.addBlip(F.x, F.y, F.z, 52, 2, "Robbable Store")
        end
...
Citizen.CreateThread(
    function()
        for v, w in pairs(r) do
            local G = tFR.loadModel(w.shopNpcModel)
            local H =
                CreatePed(
                26,
                G,
            ...
            SetPedFleeAttributes(H, 0, 0)
            local I = tFR.loadModel(w.prop_safe)
            local J = tFR.loadModel(w.prop_door)
            local K = tFR.loadModel(w.money_prop)
            local L = tFR.loadModel(w.money_prop2)
            local M = tFR.loadModel(w.money_prop3)
            local N = tFR.loadModel(w.money_prop4)
        ...
        while true do
            local U = GetEntityCoords(tFR.getPlayerPed())
            for v, w in pairs(r) do
                             ...
                             then
                                if not q then
                                    if GetSelectedPedWeapon(tFR.getPlayerPed()) == GetHashKey("WEAPON_UNARMED") then
                                        tFR.notify("~r~You need a weapon in your hands to rob this store!")
                                    else
                                        TriggerServerEvent("FR:initiateStoreRobbery", v)
...
RegisterNetEvent(
    "FR:beginStoreRobbingAnimations",
    function(v)
...
RegisterNetEvent(
    "FR:storeRobberyInProgress",
    function(Y, v)
        q = Y
        local Z = GetEntityCoords(tFR.getPlayerPed())
        while q do
            local U = GetEntityCoords(tFR.getPlayerPed())
            if #(Z - U) > 20 then
                TriggerServerEvent("FR:forceEndRobbery", v)
...
RegisterNetEvent(
    ...
    function(v)
        ...
        local G = tFR.loadModel(r[v].shopNpcModel)
        local H =
            CreatePed(
            26,
            G,
...
function RunMiniGame(v, ab, ac)
    ...
    while safeCrackingInProgress do
        ...
        elseif safeCrackingState == "cracking" then
            ...
            local ae = GetEntityHealth(tFR.getPlayerPed())
            if ae <= 102 or tFR.isHandcuffed() then
                ...
                if IsControlJustPressed(0, 172) then
                    if af == a[ad] then
                        ...
                        if IsSafeUnlocked(b, ad) then
                            ...
                            safeCrackingInProgress = false
                            ClearPedTasksImmediately(tFR.getPlayerPed())
                        end
                    else
                        ad = 1
                        b = InitSafeLocks(a)
...
function GetSafeDoorAnimOffsetPosition(ah, ai, aj)
    ...
    return vector3(ah.x + ao, ah.y + ap, GetEntityCoords(tFR.getPlayerPed()).z)
end
function PlaySafeCrackIntroAnim(aq, h)
    local ar = "mini@safe_cracking"
    local as = "step_into"
    loadAnimDict(ar)
    TaskPlayAnimAdvanced(tFR.getPlayerPed(), ar, as, aq.x, aq.y, aq.z, 0.0, 0.0, h, 8.0, 8.0, -1, 2, 0.7, 0, 0)
    Wait(0)
    Wait(1000)
    safeCrackingState = "cracking"
end
function HandleSafeDialMovement()
    local at = tFR.getPlayerPed()
    local ar = "mini@safe_cracking"
    local au = ""
    ...
    loadAnimDict(ar)
    if not IsEntityPlayingAnim(tFR.getPlayerPed(), ar, au, 3) then
        TaskPlayAnimAdvanced(tFR.getPlayerPed(), ar, au, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    end
end
function HandleIncorrectMovement(ad, b)
    ...
    loadAnimDict(ar)
    TaskPlayAnimAdvanced(tFR.getPlayerPed(), ar, ax, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    d = "rotation.idle"
end
...
function EndMiniGame(v, ab)
    ...
    Wait(2500)
    TriggerServerEvent("FR:syncOpenSafeDoor", v)
    safeCrackingState = "setup"
