...
Citizen.CreateThread(
    function()
        Wait(1000)
        TriggerServerEvent("FR:requestCoinflipTableData")
        for q = 0, #c.coinflipTables do
            local D = c.coinflipTables[q]
            local E = tFR.loadModel(D.tableModel)
            ...
            if d then
                ...
                if
                    not l and IsControlJustPressed(0, 202) and not g and
                        not RageUI.Visible(RMenu:Get("frcoinflip", "confirm"))
                 then
                    local L = tFR.loadAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                    TaskPlayAnim(tFR.getPlayerPed(), L, "sit_exit_left", 1.0, 1.0, 2500, 0, 0.0, false, false, false)
                    RemoveAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                    d = false
                    e = true
                    showHowToCoinflip(false)
                    TriggerServerEvent("FR:cancelCoinflip", true)
                    TriggerServerEvent("FR:leaveCoinflipTable")
                    h = {}
                    m = false
                    SetTimeout(
                        5000,
            ...
            if not d then
                if a ~= -1 and b < 2 then
                    if IsControlJustPressed(0, 38) then
                        if f[a] == false then
                            TriggerServerEvent("FR:requestSitAtCoinflipTable", a)
                        end
                    end
                end
            end
...
function goToCoinflipSeat(o)
    ...
    TaskGoStraightToCoord(tFR.getPlayerPed(), S.x, S.y, S.z, 1.0, 5000, T.z, 0.01)
    Wait(500)
    local U = v(o)
    local V = A(o)
...
function showHowToCoinflip(X)
    ...
    RageUI.Visible(RMenu:Get("frcoinflip", "instructions"), X)
end
function showCoinflipMenu(X)
    RageUI.ActuallyCloseAll()
    RageUI.Visible(RMenu:Get("frcoinflip", "mainmenu"), X)
end
RMenu.Add(
    "frcoinflip",
    "instructions",
...
RageUI.CreateWhile(
    ...
    function()
        RageUI.IsVisible(
            RMenu:Get("frcoinflip", "instructions"),
            true,
            false,
            true,
            function()
                    ...
                    function(Y, Z, _)
                        if _ then
                            ...
                            if tonumber(a0) then
                                a0 = tonumber(a0)
                                if a0 > 0 then
                                    TriggerServerEvent("FR:proposeCoinflip", a0)
                                end
                            end
                        end
                    end
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(Y, Z, _)
                                if _ then
                                    ...
                                    if M.user_id == tFR.getUserId() then
                                        k = true
                                    else
                                        k = false
                                    end
            ...
            function()
                ...
                if k then
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(Y, Z, _)
                            if _ then
                                TriggerServerEvent("FR:cancelCoinflip")
                            end
                        end,
                        RMenu:Get("frcoinflip", "mainmenu")
                    )
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(Y, Z, _)
                            if _ then
                                TriggerServerEvent("FR:acceptCoinflip", i)
                            end
                        end,
                        RMenu:Get("frcoinflip", "mainmenu")
                    )
...
RegisterNetEvent(
    "FR:addCoinflipProposal",
    function(a1, M)
        h[a1] = M
    end
)
RegisterNetEvent(
    "FR:cancelCoinflipBet",
    function(a1)
        h[a1] = nil
    end
)
RegisterNetEvent(
    "FR:clearCoinflipBets",
    function(a1)
        h = {}
    end
)
RegisterNetEvent("FR:coinflipOutcome",function(a4, a5)
        l = true
        ...
        SetTimeout(10000,function()
                a7 = true
                if a4 then
                    tFR.notify("~g~ You won £" .. getMoneyStringFormatted(a5.amount) .. "!")
                    SendNUIMessage({transactionType = "playCasinoWin"})
                else
                    tFR.notify("~r~ You lost £" .. getMoneyStringFormatted(a5.amount) .. "!")
