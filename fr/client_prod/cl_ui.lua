...
local l = 10.0
local m = {0, 0, 0, 255}
function tFR.setMaxUnderWaterUITimenewTime(n)
    l = n
end
local function o(p, q, r, s, t, u, v, w)
    DrawRect(p + r / 2, q + s / 2, r, s, t, u, v, w)
end
function tFR.getCachedMinimapAnchor()
    return k
end
function tFR.getCachedResolution()
    ...
    function()
        while true do
            if not globalHideUi then
                if not globalHideEmergencyCallUI and not glifeheathui then
                    local x = tFR.getPlayerPed()
                    ...
                    local E = tFR.getShowHealthPercentageFlag()
                    if C > 1.0 then
                        C = 1.0
                    end
...
function tFR.setHealthUIType(a)
    F = true
    if a == "FR" then
        glifeheathui = false
    elseif a == "GLife" then
