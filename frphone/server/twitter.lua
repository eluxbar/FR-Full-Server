  ...
  getUser(username, password, function (user)
    ...
    MySQL.Async.insert("INSERT INTO twitter_tweets (`authorId`, `message`, `realUser`) VALUES(@authorId, @message, @realUser);", {
      ...
      MySQL.Async.fetchAll('SELECT * from twitter_tweets WHERE id = @id', {
        ...
        TriggerClientEvent('FR:twitter_newTweets', -1, tweet)
        TriggerEvent('FR:twitter_newTweets', tweet)
      end)
    end)
  end)
end
        ...
        if (row[1] == nil) then
          MySQL.Async.insert('INSERT INTO twitter_likes (`authorId`, `tweetId`) VALUES(@authorId, @tweetId)', {
            ...
            MySQL.Async.execute('UPDATE `twitter_tweets` SET `likes`= likes + 1 WHERE id = @id', {
              ...
              TriggerClientEvent('FR:twitter_updateTweetLikes', -1, tweet.id, tweet.likes + 1)
              TriggerClientEvent('FR:twitter_setTweetLikes', sourcePlayer, tweet.id, true)
              TriggerEvent('FR:twitter_updateTweetLikes', tweet.id, tweet.likes + 1)
            end)    
          end)
        else
          MySQL.Async.execute('DELETE FROM twitter_likes WHERE id = @id', {
            ...
            MySQL.Async.execute('UPDATE `twitter_tweets` SET `likes`= likes - 1 WHERE id = @id', {
              ...
              TriggerClientEvent('FR:twitter_updateTweetLikes', -1, tweet.id, tweet.likes - 1)
              TriggerClientEvent('FR:twitter_setTweetLikes', sourcePlayer, tweet.id, false)
              TriggerEvent('FR:twitter_updateTweetLikes', tweet.id, tweet.likes - 1)
            end)
          end)
        end
...
function TwitterShowError (sourcePlayer, title, message)
  TriggerClientEvent('FR:twitter_showError', sourcePlayer, message)
end
function TwitterShowSuccess (sourcePlayer, title, message)
  TriggerClientEvent('FR:twitter_showSuccess', sourcePlayer, title, message)
end

RegisterServerEvent('FR:twitter_login')
AddEventHandler('FR:twitter_login', function(username, password)
  ...
  getUser(username, password, function (user)
    if user == nil then
      ...
      TriggerClientEvent('FR:twitter_setAccount', sourcePlayer, username, password, user.authorIcon)
    end
  end)
end)

RegisterServerEvent('FR:twitter_changePassword')
AddEventHandler('FR:twitter_changePassword', function(username, password, newPassword)
  local sourcePlayer = tonumber(source)
  getUser(username, password, function (user)
    if user == nil then
      ...
      MySQL.Async.execute("UPDATE `twitter_accounts` SET `password`= @newPassword WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
        ...
        if (result == 1) then
          TriggerClientEvent('FR:twitter_setAccount', sourcePlayer, username, newPassword, user.authorIcon)
...
RegisterServerEvent('FR:twitter_createAccount')
AddEventHandler('FR:twitter_createAccount', function(username, password, avatarUrl)
  local sourcePlayer = tonumber(source)
  TwitterCreateAccount(username, password, avatarUrl, function (id)
    if (id ~= 0) then
      TriggerClientEvent('FR:twitter_setAccount', sourcePlayer, username, password, avatarUrl)
...
RegisterServerEvent('FR:twitter_getTweets')
AddEventHandler('FR:twitter_getTweets', function(username, password)
  local sourcePlayer = tonumber(source)
  if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
    getUser(username, password, function (user)
      local accountId = user and user.id
      TwitterGetTweets(accountId, function (tweets)
        TriggerClientEvent('FR:twitter_getTweets', sourcePlayer, tweets)
      end)
    end)
  else
...
RegisterServerEvent('FR:twitter_getFavoriteTweets')
AddEventHandler('FR:twitter_getFavoriteTweets', function(username, password)
  local sourcePlayer = tonumber(source)
  if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
    getUser(username, password, function (user)
      local accountId = user and user.id
      TwitterGetFavotireTweets(accountId, function (tweets)
        TriggerClientEvent('FR:twitter_getFavoriteTweets', sourcePlayer, tweets)
      end)
    end)
  else
...
RegisterServerEvent('FR:twitter_postTweets')
AddEventHandler('FR:twitter_postTweets', function(username, password, message)
  local sourcePlayer = tonumber(source)
  local srcIdentifier = getPlayerID(source)
  TwitterPostTweet(username, password, message, sourcePlayer, srcIdentifier)
end)

RegisterServerEvent('FR:twitter_toogleLikeTweet')
AddEventHandler('FR:twitter_toogleLikeTweet', function(username, password, tweetId)
  local sourcePlayer = tonumber(source)
  TwitterToogleLike(username, password, tweetId, sourcePlayer)
end)


RegisterServerEvent('FR:twitter_setAvatarUrl')
AddEventHandler('FR:twitter_setAvatarUrl', function(username, password, avatarUrl)
  local sourcePlayer = tonumber(source)
