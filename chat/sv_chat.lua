...
local function routeMessage(source, author, message, mode, fromConsole)
    if source >= 1 then
        author = exports["fr"]:GetDiscordName(source)
    end
    ...
    if mode == "OOC" then
        TriggerEvent("FR:ooc", source, message)
        return
    end
    if mode == "Gang" then 
    ...
    if mode == "Anonymous" then
        TriggerEvent("FR:Anon", source, message)
        return
    end
    if mode == "Police" then
    ...
    if mode == "Admin" then
        for k,v in ipairs(GetPlayers()) do
            if exports['fr']:hasPermission(v, "admin.tickets") then
...
AddEventHandler('_chat:messageEntered', function(author, color, message, mode)
    ...
    if not WasEventCanceled() then
        if cooldown[source] and not (os.time() > cooldown[source]) then
            TriggerClientEvent('chatMessage', source, "FR",  { 255, 255, 255 }, "You are being rate limited.", "alert", mode)
            return
        else
            cooldown[source] = nil
        end
...
AddEventHandler('__cfx_internal:commandFallback', function(command)
    local name = exports["fr"]:GetDiscordName(source)
